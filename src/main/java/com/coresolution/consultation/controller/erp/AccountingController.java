...
/**
 * 회계 분개 Controller 표준 문서: docs/standards/ERP_ADVANCEMENT_STANDARD.md API 설계 표준:
...
public class AccountingController extends BaseApiController {
    ...
    /**
     * 분개 생성 POST /api/v1/erp/accounting/entries 표준 문서: docs/standards/ERP_ADVANCEMENT_STANDARD.md
     */
    @PostMapping
    public ResponseEntity<?> createJournalEntry(@RequestBody JournalEntryCreateRequest request,
            HttpSession session) {
    ...
    /**
     * 분개 목록 조회 GET /api/v1/erp/accounting/entries 표준 문서: docs/standards/ERP_ADVANCEMENT_STANDARD.md
     */
    @GetMapping
    ...
    /**
     * 분개 상세 조회 GET /api/v1/erp/accounting/entries/{id} 표준 문서:
    ...
    /**
     * 분개 승인 POST /api/v1/erp/accounting/entries/{id}/approve 표준 문서:
     * docs/standards/ERP_ADVANCEMENT_STANDARD.md
     */
    @PostMapping("/{id}/approve")
    public ResponseEntity<?> approveJournalEntry(@PathVariable Long id,
            @RequestBody JournalEntryApproveRequest request, HttpSession session) {
        ResponseEntity<?> accessCheck = checkErpAccess(session);
        if (accessCheck != null) {
            return accessCheck;
        }
    ...
    /**
     * 분개 전기 POST /api/v1/erp/accounting/entries/{id}/post 표준 문서:
     * docs/standards/ERP_ADVANCEMENT_STANDARD.md
     */
    @PostMapping("/{id}/post")
    ...
    /**
     * 분개 수정 (DRAFT 상태에서만 가능) PUT /api/v1/erp/accounting/entries/{id} 표준 문서:
     * docs/standards/ERP_ADVANCEMENT_STANDARD.md
     */
    @PutMapping("/{id}")
    public ResponseEntity<?> updateJournalEntry(@PathVariable Long id,
            @RequestBody JournalEntryCreateRequest request, HttpSession session) {
        ResponseEntity<?> accessCheck = checkErpAccess(session);
        if (accessCheck != null) {
            return accessCheck;
        }
    ...
    public static class JournalEntryCreateRequest {
        ...
        public AccountingEntry toAccountingEntry() {
            return AccountingEntry.builder().entryNumber(entryNumber).entryDate(entryDate)
    ...
    public static class JournalEntryLineRequest {
        ...
        public JournalEntryLine toJournalEntryLine() {
            return JournalEntryLine.builder().accountId(accountId)
