...
# Private 저장소에서도 동작하도록 raw.githubusercontent.com 비인증 다운로드를 쓰지 않습니다.
...
DB_NAME="mindgarden"
REMOTE_SQL="/tmp/add_mapping_permissions.sql"

echo "📡 운영 서버: $SERVER"
echo "🗄️ 데이터베이스: $DB_NAME"
echo ""

# 로컬 체크아웃 기준 SQL (스크립트 위치: deployment/)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SQL_FILE="${SCRIPT_DIR}/../src/main/resources/sql/add_mapping_permissions.sql"
...
# 사용자에게 확인 요청
echo "⚠️  다음 작업을 수행합니다:"
echo "   1. 로컬 SQL을 서버로 복사 (scp)"
echo "   2. permissions 테이블에 매핑 관련 권한 추가"
echo "   3. role_permissions 테이블에 역할별 권한 매핑 추가"
echo ""
...
echo "📤 SQL 파일 업로드 중..."
echo "================================"
scp "$SQL_FILE" "mindgarden@${SERVER}:${REMOTE_SQL}"
...
ssh mindgarden@$SERVER << ENDSSH
    ...
    if [ ! -f ${REMOTE_SQL} ]; then
        echo "❌ SQL 파일이 서버에 없습니다: ${REMOTE_SQL}"
        exit 1
    fi
    ...
    echo "🗄️ MySQL 접속 중..."
    echo "비밀번호를 입력하세요:"
    mysql -u mindgarden -p mindgarden < ${REMOTE_SQL}
