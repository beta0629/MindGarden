...
BEGIN
    ...
    DECLARE v_quarterly_breakdown JSON;
    ...
    BEGIN
        ...
        SET p_success = FALSE;
        SET p_message = CONCAT('연도별 재무 보고서 생성 중 오류 발생: ', v_error_message);
    ...
    -- 4. 분기별 연도 내 상세 (테넌트 격리)
    SELECT JSON_ARRAYAGG(
        JSON_OBJECT(
            'quarter', QUARTER(ft.transaction_date),
            'quarterly_revenue', COALESCE(SUM(CASE WHEN ft.transaction_type = 'INCOME' THEN ft.amount ELSE 0 END), 0),
            'quarterly_expenses', COALESCE(SUM(CASE WHEN ft.transaction_type = 'EXPENSE' THEN ft.amount ELSE 0 END), 0),
            'quarterly_transactions', COUNT(*)
        )
    )
    INTO v_quarterly_breakdown
    FROM financial_transactions ft
    WHERE ft.tenant_id = p_tenant_id
      AND ft.transaction_date BETWEEN start_date AND end_date
      AND ft.is_deleted = FALSE
    GROUP BY QUARTER(ft.transaction_date)
    ORDER BY QUARTER(ft.transaction_date);
    
    -- 5. 최종 보고서 데이터 생성
    SET p_report_data = JSON_OBJECT(
        'summary', IFNULL(v_summary, JSON_OBJECT()),
        'quarterly_breakdown', IFNULL(v_quarterly_breakdown, JSON_ARRAY())
    );
    
    SET p_success = TRUE;
    SET p_message = '연도별 재무 보고서 생성이 완료되었습니다.';
