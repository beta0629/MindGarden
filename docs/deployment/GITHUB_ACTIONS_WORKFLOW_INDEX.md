...
| 파일 | 역할 | 트리거·비고 |
|------|------|-------------|
| [`deploy.yml`](../../.github/workflows/deploy.yml) | 수동 배포 허브 — `env`×`target`으로 개별 `deploy-*.yml`을 `gh workflow run` 디스패치 | `workflow_dispatch`만. **path push 없음**. 기본 `deploy_ref`: prod=`release/prod`, dev=`release/dev`. gap(예: ops-be/dev, onboarding/prod, nginx/prod, procedures-mysql/dev, core-prod-unified/dev)은 fail-fast. |
...
|------|-----------|-----------------|--------------|----------------|----------|
| Core 개발 | [`deploy-frontend-dev.yml`](../../.github/workflows/deploy-frontend-dev.yml) | `release/dev` push, `workflow_dispatch` | `frontend/**`, 해당 workflow | `DEV_SERVER_HOST` / `DEV_SERVER_USER` / `DEV_SERVER_SSH_KEY` → 재사용 워크플로 `ssh_*` | `https://dev.core-solution.co.kr` (실패 시 경고만, exit 0) |
| Trinity 개발 | [`deploy-trinity-dev.yml`](../../.github/workflows/deploy-trinity-dev.yml) | `release/dev` push, `workflow_dispatch` | `frontend-trinity/**`, 해당 workflow | 동일 | `https://apply.dev.e-trinity.co.kr`, `https://dev.e-trinity.co.kr` (순차, 하나 성공 시 통과; 전부 실패 시 경고만) |
| Trinity 운영 | [`deploy-trinity-prod.yml`](../../.github/workflows/deploy-trinity-prod.yml) | `release/prod` push, `workflow_dispatch`(`deploy_ref`, release/prod만 허용) | 동일 | `PRODUCTION_HOST` / `PRODUCTION_USER` / `PRODUCTION_SSH_KEY` | `https://apply.e-trinity.co.kr` |
| Ops 개발 | [`deploy-ops-dev.yml`](../../.github/workflows/deploy-ops-dev.yml) | `release/dev` push, `workflow_dispatch` | `frontend-ops/**`, 해당 workflow | `DEV_SERVER_*` | `https://ops.dev.e-trinity.co.kr` |
| Ops 운영 | [`deploy-ops-prod.yml`](../../.github/workflows/deploy-ops-prod.yml) | `release/prod` push, `workflow_dispatch`(release/prod 가드) | 동일 | `PRODUCTION_*` | `https://ops.e-trinity.co.kr` |
...
| `deploy-frontend-dev.yml` | Core 메인 프론트 → html-dev | `release/dev`+paths, dispatch | — / [`deploy-production.yml`](../../.github/workflows/deploy-production.yml)(코어 통합) | 사용 | |
| `deploy-trinity-dev.yml` | Trinity 프론트 개발 | `release/dev`+paths, dispatch | [`deploy-trinity-prod.yml`](../../.github/workflows/deploy-trinity-prod.yml) | 사용 | |
| `deploy-trinity-prod.yml` | Trinity 프론트 운영 | `release/prod`+paths, dispatch | 상위와 쌍 | 사용 | |
| `deploy-ops-dev.yml` | Ops 프론트 개발 | `release/dev`+paths, dispatch | [`deploy-ops-prod.yml`](../../.github/workflows/deploy-ops-prod.yml) | 사용 | |
| `deploy-ops-prod.yml` | Ops 프론트 운영 | `release/prod`+paths, dispatch | 상위와 쌍 | 사용 | |
...
| `deploy-backend-dev.yml` | 코어 백엔드 개발 | `release/dev`+세분 paths | — / `deploy-production.yml` | — | 온보딩 경로 제외 |
| `deploy-onboarding-dev.yml` | 온보딩 개발 | `release/dev`+paths | — | — | |
| `deploy-nginx-dev.yml` | Nginx 설정 개발 | `release/dev`+`config/nginx/**` | — | — | |
| `deploy-production.yml` | 코어 운영 통합 | `release/prod` push(코어 Java 온보딩 제외·`pom`·`db/migration`·`deployment/application-production.yml`·`sql/**`·`database/schema/**`·`config/nginx/**`·해당 workflow), `workflow_dispatch`(`deploy_ref`, release/prod 가드) | — | — | 백엔드: **블루그린**(비활성 슬롯 기동·헬스·Nginx upstream 스니펫·`reload`) — [PRODUCTION_BLUE_GREEN_BACKEND_CUTOVER.md](./PRODUCTION_BLUE_GREEN_BACKEND_CUTOVER.md). push 시 checkout `github.sha` |
| `deploy-frontend-prod.yml` | Core CRA 프론트만 운영 정적 배포 | `release/prod`+`frontend/**`, dispatch(`deploy_ref`, release/prod 가드) | — | — | JAR·DB 없이 SPA만 갱신; `deploy-production`과 별도 |
| `deploy-ops-backend-prod.yml` | Ops 백엔드 운영 | `release/prod`+paths, dispatch | — | — | |
...
| `deploy-procedures-dev.yml` | 표준 프로시저 개발 | `release/dev`+DB paths | [`deploy-procedures-prod.yml`](../../.github/workflows/deploy-procedures-prod.yml) | — | |
...
| `deploy-mobile.yml` | 모바일 빌드 | `release/prod`+`mobile/**`, dispatch | — | — | |
