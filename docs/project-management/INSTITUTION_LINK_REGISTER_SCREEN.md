...
최가을 유형. 지금은 **연계만**: 기관 정보·내담자·초기 상담 선납. 월결제·결제 주기는 후속(고정 배지 금지). 이후 정보 확인·월말 상담내역 문서용. **스펙만. 구현 없음.**
...
| 내담자 | 내담자 연결 | 검색 → 선택 | ● | 아래 |
| 선납 | 초기 상담 선납 | BadgeSelect `선납함` / `선납 안 함` | ● | 기본 미선택 |
| 선납 | 선납 일자 | FormInput date | 선납함일 때만 ● | `선납 안 함`이면 행 숨김 |
| 선납 | 선납 금액 | FormInput | 선납함일 때만 ● | 위와 동일 |
...
월결제, 결제 주기(월 단위 고정 배지 포함), 바우처, 회기 잔여 입력, 패키지, 상담사, 주소·사업자번호·메모·첨부, 월말 문서 미리보기, 스테퍼, KPI.
...
- [ ] 필드 = §3 표만. 월결제·결제 주기·바우처·잔여 회기 없음
