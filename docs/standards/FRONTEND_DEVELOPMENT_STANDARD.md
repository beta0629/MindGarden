...
- [Expo 네이티브 앱 하드코딩·색상 표준](./EXPO_APP_HARDCODING_AND_COLORS.md) — `expo-app/`(React Native) 전용. **색상 리터럴은 토큰·브랜드 상수 파일 외 금지**
...
- **아이콘(lucide-react)**: 신규 아이콘 사용 시 [lucide.dev/icons](https://lucide.dev/icons) 또는 패키지 export 목록으로 **실제 export 여부 확인** 후 import (미확인 시 빌드 오류 가능, 참고: [docs/debug/BUILD_LAYOUTGRID2_IMPORT_FIX.md](../debug/BUILD_LAYOUTGRID2_IMPORT_FIX.md))
