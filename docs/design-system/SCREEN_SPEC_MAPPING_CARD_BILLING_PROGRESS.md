...
- **아코디언**: 기본 접힘. 헤더 `일정 상세` + `일정 N건` (occupying 건수). 0건이면 섹션 미노출
...
- **상담일시(Peek)**: `consultationSchedules[]` (mappingId 점유 SSOT, **COMPLETED 포함**). 월별 한눈·행 포맷은 §3.2와 동일
...
- [ ] 기관연동 카드: `이 연동 누적 N회` 만. used/total·「상태 불일치」·boolean 「이력 있음」 단독 없음
- [ ] Side Peek: 일정 상세 아코디언에서 한눈 월별 일시 + 행 목록. `N월 M/D` 월중복 금지. 다른 매핑 날짜 혼입 없음
- [ ] Side Peek 상태 행: 「기관연동」 배지 **1개** (mute/본문에 동일 문구 이중 렌더 금지)
- [ ] React #130 방어(`safeDisplay` / `SafeText`)
- [ ] 하드코딩 색·매직 문구 최소화(상태 라벨 상수화)
- [ ] 단위 테스트: 진행 문구·IL 매핑 스코프·Peek 아코디언·빈 목록·객체 방어
