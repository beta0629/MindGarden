...
**버전**: 1.0.0  
**작성**: core-designer  
**참조**: PENCIL_DESIGN_GUIDE.md, 어드민 대시보드 샘플 (https://mindgarden.dev.core-solution.co.kr/admin-dashboard-sample)  
...
- **배지 간격**: 가로·세로 gap 8px~12px (`var(--cs-spacing-sm)` ~ `var(--cs-spacing-md)`).
...
- 배지 영역이 **스크롤과 함께 보일 때**: 섹션 블록 패딩(24px) 내부에 두고, 배지 그룹 상하에 12~16px 여백. 라벨은 배지 그룹 위 8px.
- **정렬**: 라벨은 왼쪽 정렬, 배지 그룹은 라벨과 동일한 시작선(좌측 정렬). 폼이 좌측 라벨 + 우측 필드 구조면 배지 영역은 필드 셀에 배치.
- **섹션 블록**: `docs/design-system/PENCIL_DESIGN_GUIDE.md`의 섹션 블록(배경 #F5F3EF, 테두리 1px #D4CFC8, radius 16px, 패딩 24px) 안에 들어가면 블록 내부 gap 16px 유지.
...
- [ ] 모달/폼 내부에서 배지 영역 여백·정렬이 섹션 블록과 맞는가?
