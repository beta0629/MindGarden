...
- ✅ `PRINCIPAL` (원장) 역할 추가됨
- ✅ `OWNER` (사장) 역할 추가됨
- ✅ `TENANT_ADMIN` (테넌트관리자) 역할 추가됨
- 🔄 향후: UserRoleAssignment 기반 동적 역할 시스템으로 전환 예정
...
**현재 구현된 역할들:**
- `PRINCIPAL`: 원장 (학원 최고 관리자)
- `OWNER`: 사장 (사업체 대표)
- `TENANT_ADMIN`: 테넌트관리자 (테넌트 단위 관리자)
...
UserRole enum (현재 사용)
  ├─ ADMIN: 관리자
  ├─ BRANCH_MANAGER: 지점장
  ├─ BRANCH_SUPER_ADMIN: 지점 수퍼 어드민
  ├─ PRINCIPAL: 원장 ⭐ (신규 추가)
  ├─ OWNER: 사장 ⭐ (신규 추가)
  ├─ TENANT_ADMIN: 테넌트관리자 ⭐ (신규 추가)
  └─ 기타 기존 역할들...
```
...
UserRoleAssignment (신규)
  ...
  ├─ effective_from/to: 유효 기간
  └─ is_active: 활성 상태

User (현재)
  └─ role: UserRole enum (레거시 호환용)
```
...
- 상담소: 수퍼관리자 (옵션)
- 학원: 원장 (옵션) - `PRINCIPAL` 역할
- 기타: 원장 (옵션) - `OWNER` 역할

#### 레벨 5: 테넌트 관리자 (신규)
- 모든 업종: 테넌트관리자 - `TENANT_ADMIN` 역할
- 테넌트 단위 최고 권한 보유
