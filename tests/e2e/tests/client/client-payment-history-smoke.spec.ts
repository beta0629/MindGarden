/**
 * 내담자 결제 내역 화면 스모크: `/client/payment-history` 진입·루트 testid 가시성
 *
 * 자격 증명 SSOT: `getClientWebLogin()` — 기본 `01086322121` / `godgod826!` (`.cursor/skills/core-solution-testing/SKILL.md`)
 *
 * testid (`ClientPaymentHistory.js`): `client-payment-history-page`
 */
...
test.describe('내담자 결제 내역 화면 스모크', () => {
  ...
  test('/client/payment-history 에서 페이지 testid가 보인다', async ({
    page,
  }: {
    page: Page;
  }) => {
    await page.goto('/client/payment-history', { waitUntil: 'domcontentloaded' });
    await expect(page).toHaveURL(/\/client\/payment-history/, { timeout: 15_000 });

    const pageRoot = page.getByTestId('client-payment-history-page');
