...
**ADMIN-HIDE-CARDS-SSOT** 위임 후 `HIDE_ADMIN_CARD_IDS`가 상수 파일로 정리되면, 「관리 기능」카드 노출 확인은 **38번** 및 상단·다음 단계 스모크와 **동일 경로·동일 기대**면 생략한다.

---

### E2E 선행 조건 (로컬)

Playwright ERP·관리자 로그인 스펙이 **로그인 URL·`waitForURL` 타임아웃**일 때 아래를 SSOT로 둔다. 자격 증명은 [운영 반영 전 체크리스트 — 문서 계층·보안](../../운영반영/PRE_PRODUCTION_GO_LIVE_CHECKLIST.md)에 맞춰 **시크릿·실계정 평문을 문서·PR에 넣지 않는다**(예시는 placeholder·변수명만). 변수 우선순위·로컬 기본값은 [core-solution-testing 스킬](../../../.cursor/skills/core-solution-testing/SKILL.md)·[`tests/e2e/helpers/erpAuth.ts`](../../../tests/e2e/helpers/erpAuth.ts)를 따른다.

1. **`BASE_URL`**: Playwright `use.baseURL`과 동일(기본 `http://localhost:3000`). 설정·`webServer`는 [`tests/e2e/playwright.config.ts`](../../../tests/e2e/playwright.config.ts) SSOT.
2. **기동**: 백엔드 API **8080** + 프론트 **3000**이 함께 떠 있고, 프론트가 API를 호출하는 베이스 URL(`REACT_APP_API_BASE_URL` 등)이 **8080**과 정합인지 확인한다.
3. **로그인 환경 변수**: `E2E_TEST_EMAIL` / `E2E_TEST_PASSWORD`를 최우선으로 두고, 없으면 `TEST_USERNAME` / `TEST_PASSWORD` 폴백(값은 Secrets·로컬 전용 env에만).
4. **계정**: **ERP 권한**(관리자 LNB·`/erp/*` 접근)이 있는 계정이어야 한다.
5. **타임아웃 시 점검 순서**: (1) 8080·인증 API 응답 (2) `BASE_URL`·3000·Playwright `webServer` 기동 (3) 위 자격 증명·ERP 권한 (4) 네트워크·trace에서 `/api/v1/auth` 등 실패 원인 — 수동 회귀 번호는 본 문서 상단 절·「다음 단계」와 **중복 링크만** 활용한다.

**배포 직후 검증**(무중단 컷오버·운영 릴리스 직후): [ZERO_DOWNTIME_POST_DEPLOY_CHECKLIST.md](./ZERO_DOWNTIME_POST_DEPLOY_CHECKLIST.md).

**복사 실행 절차(터미널 순서)**: [다음 세션 로컬 검증 명령](../../project-management/NEXT_LOCAL_VERIFICATION_COMMANDS.md).
...
변경일: 2026-04-28 — 「다음 단계」 직전 **E2E 선행 조건 (로컬)** 추가(Playwright 로그인 URL 타임아웃 시 온보딩 SSOT).
