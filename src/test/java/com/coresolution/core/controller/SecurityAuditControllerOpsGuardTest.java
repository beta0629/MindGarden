...
/**
 * {@link SecurityAuditController} OPS 가드 회귀 테스트 (Phase 3 — ops-portal-migration).
 *
 * @author CoreSolution
 * @since 2026-06-15
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("SecurityAuditController OPS 가드 회귀 (Phase 3)")
class SecurityAuditControllerOpsGuardTest {
    ...
    void setUp() throws Exception {
        ...
        SecurityAuditController controller = new SecurityAuditController(
            auditLogRepository, opsTenantConstants);
    ...
    void classLevelPreAuthorize_isOps() {
        PreAuthorize annotation = SecurityAuditController.class.getAnnotation(PreAuthorize.class);
    ...
    @Test
    @DisplayName("[#1] OPS + HQ 테넌트 → /recent 정상")
    void hybrid_opsRole_hqTenant_recentOk() throws Exception {
        TenantContextHolder.setTenantId(HQ_TENANT_ID);
        mockMvc.perform(get("/api/v1/security/audit/recent").param("limit", "5"));
    }

    @Test
    @DisplayName("[#2] OPS + 외부 테넌트 → /recent AccessDenied")
    ...
    @Test
    @DisplayName("[#3] OPS + 외부 테넌트 → /summary 차단")
    void hybrid_opsRole_externalTenant_summaryBlocked() {
        TenantContextHolder.setTenantId(EXTERNAL_TENANT_ID);
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->
            mockMvc.perform(get("/api/v1/security/audit/summary")))
            .satisfies(e -> {
                Throwable root = e;
                while (root.getCause() != null && root.getCause() != root) {
                    root = root.getCause();
    ...
    @Test
    @DisplayName("[#4] OPS + 외부 테넌트 → /statistics 차단")
    void hybrid_opsRole_externalTenant_statisticsBlocked() {
        TenantContextHolder.setTenantId(EXTERNAL_TENANT_ID);
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->
            mockMvc.perform(get("/api/v1/security/audit/statistics")))
            .satisfies(e -> {
                Throwable root = e;
                while (root.getCause() != null && root.getCause() != root) {
                    root = root.getCause();
    ...
    @Test
    @DisplayName("[#5] OPS + 외부 테넌트 → /by-event-type 차단")
    void hybrid_opsRole_externalTenant_byEventTypeBlocked() {
        TenantContextHolder.setTenantId(EXTERNAL_TENANT_ID);
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->
            mockMvc.perform(get("/api/v1/security/audit/by-event-type")
    ...
    @Test
    @DisplayName("[#6] HQ_MASTER 잔존 @PreAuthorize 표현식 0 — 정리 회귀 (Javadoc 제외)")
    void hybrid_hqMasterExpression_eliminated() throws Exception {
        java.nio.file.Path source = java.nio.file.Path.of(
            "src/main/java/com/coresolution/core/controller/SecurityAuditController.java");
