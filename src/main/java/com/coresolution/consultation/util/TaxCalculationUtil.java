...
public class TaxCalculationUtil {
     ...
     * @param vatRate 부가세율 (SSOT 조회값)
     * @return 부가세 금액
     */
    public static BigDecimal calculateVatFromAmountIncludingTax(
            BigDecimal amountIncludingTax, BigDecimal vatRate) {
        ...
        return amountIncludingTax.multiply(vatRate)
                .divide(BigDecimal.ONE.add(vatRate), 0, RoundingMode.HALF_UP);
    }
     ...
     * @return 부가세 제외 금액
     */
    public static BigDecimal calculateAmountExcludingTax(
            BigDecimal amountIncludingTax, BigDecimal vatRate) {
        if (amountIncludingTax == null || amountIncludingTax.compareTo(BigDecimal.ZERO) <= 0) {
            return BigDecimal.ZERO;
        }

        BigDecimal taxAmount = calculateVatFromAmountIncludingTax(amountIncludingTax, vatRate);
     ...
     * @return 부가세 금액
     */
    public static BigDecimal calculateVatFromAmountExcludingTax(
            BigDecimal amountExcludingTax, BigDecimal vatRate) {
        ...
        return amountExcludingTax.multiply(vatRate).setScale(0, RoundingMode.HALF_UP);
    }
     ...
     * @return 부가세 포함 금액
     */
    public static BigDecimal calculateAmountIncludingTax(
            BigDecimal amountExcludingTax, BigDecimal vatRate) {
        if (amountExcludingTax == null || amountExcludingTax.compareTo(BigDecimal.ZERO) <= 0) {
            return BigDecimal.ZERO;
        }

        BigDecimal taxAmount = calculateVatFromAmountExcludingTax(amountExcludingTax, vatRate);
     ...
     */
    public static TaxCalculationResult calculateTaxFromPayment(
            BigDecimal paymentAmount, BigDecimal vatRate) {
        if (paymentAmount == null || paymentAmount.compareTo(BigDecimal.ZERO) <= 0) {
            return new TaxCalculationResult(BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO);
        }

        BigDecimal vatAmount = calculateVatFromAmountIncludingTax(paymentAmount, vatRate);
     ...
     * @return TaxCalculationResult
     */
    public static TaxCalculationResult calculateTaxForExpense(
            BigDecimal expenseAmount, BigDecimal vatRate) {
        if (expenseAmount == null || expenseAmount.compareTo(BigDecimal.ZERO) <= 0) {
            return new TaxCalculationResult(BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO);
        }

        BigDecimal vatAmount = calculateVatFromAmountExcludingTax(expenseAmount, vatRate);
