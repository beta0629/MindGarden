...
describe('Consultation log view saved view (28g Phase 5D)', () => {
  ...
  it('pageId storageKey가 admin.consultation-logs.view를 사용한다', () => {
  ...
  it('named view 저장·복원 시 filters·viewMode를 유지한다', () => {
    ...
    let viewId;
    act(() => {
      viewId = result.current.saveNamedView('1월 상담일지', {
        viewMode: 'calendar',
    ...
    expect(result.current.savedView.viewMode).toBe('calendar');
