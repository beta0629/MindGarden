...
## 4. 프로필 카드 암호문 노출 (추가 이슈)

### 현상
- 내담자/스태프 목록 프로필 카드에 이름·이메일·전화가 **legacy::...** 또는 암호문 그대로 노출됨.

### 원인 (디버거)
- API가 복호화 실패 시 또는 레거시 저장값을 그대로 반환하고, 프론트에서 그대로 렌더링함.

### 수정 (기획 반영)
- **codeHelper.js**: `maskEncryptedDisplay(value, fallback)` 추가 — `legacy::` 또는 base64 패턴이면 fallback(예: '이름', '이메일', '전화번호') 반환.
- **ClientOverviewTab.js**: 내담자 카드의 name, email, phone에 `maskEncryptedDisplay` 적용.
- **StaffManagement.js**: 스태프 카드 및 "스태프로 지정" 목록의 name, email, phone에 `maskEncryptedDisplay` 적용.
...
- [ ] **내담자 등록 모달**에서 이메일 입력란 보이고 한 줄 배치 (ClientModal.css min-width 반영)
- [ ] 내담자/스태프 프로필 카드에 암호문 대신 '이름'/'이메일'/'전화번호' 등 마스킹 표시
