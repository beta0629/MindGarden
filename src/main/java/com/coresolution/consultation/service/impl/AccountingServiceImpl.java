...
/**
 * 회계 Service 구현체 표준 문서: docs/standards/ERP_ADVANCEMENT_STANDARD.md
 */
@Slf4j
@Service
...
public class AccountingServiceImpl implements AccountingService {
    ...
    public AccountingEntry createJournalEntry(String tenantId, AccountingEntry entry,
        ...
        log.info("분개 생성 완료: tenantId={}, entryNumber={}, totalDebit={}, totalCredit={}", tenantId,
                savedEntry.getEntryNumber(), totalDebit, totalCredit);
    ...
    public AccountingEntry approveJournalEntry(String tenantId, Long entryId, Long approverId,
        ...
        log.info("분개 승인 완료: tenantId={}, entryId={}, entryNumber={}", tenantId, entryId,
    ...
    public AccountingEntry postJournalEntry(String tenantId, Long entryId) {
        ...
        for (JournalEntryLine line : lines) {
            ledgerService.updateLedgerFromJournalEntry(tenantId, line.getAccountId(),
                    entry.getEntryDate(), line.getDebitAmount(), line.getCreditAmount());
        }
        ...
        log.info("분개 전기 완료: tenantId={}, entryId={}, entryNumber={}", tenantId, entryId,
                savedEntry.getEntryNumber());
    ...
    private Long getDefaultAccountId(String tenantId, String accountType) {
        try {
            ...
            Optional<CommonCode> accountCode = commonCodeService
                    .getTenantCodeByGroupAndValue(tenantId, "ERP_ACCOUNT_TYPE", accountType);
            ...
            if (accountCode.isPresent()) {
                ...
                if (code.getExtraData() != null && !code.getExtraData().trim().isEmpty()) {
                    try {
                        ...
                        if (extraData.startsWith("{") && extraData.contains("accountId")) {
                            String accountIdStr =
                                    extraData.replaceAll(".*\"accountId\"\\s*:\\s*(\\d+).*", "$1");
                        ...
                        log.warn(
                                "계정 ID 파싱 실패 (extraData): tenantId={}, accountType={}, extraData={}",
                ...
                if (code.getCodeDescription() != null
                        && !code.getCodeDescription().trim().isEmpty()) {
                    try {
                        String description = code.getCodeDescription().trim();
                        String accountIdStr =
                                description.contains(":") ? description.split(":")[1].trim()
                                        : description.trim();
                        return Long.parseLong(accountIdStr);
                    } catch (NumberFormatException e) {
                        log.warn(
                                "계정 ID 파싱 실패 (codeDescription): tenantId={}, accountType={}, description={}",
    ...
    private String generateEntryNumber(String tenantId) {
        ...
        Integer maxSequence =
                accountingEntryRepository.findMaxSequenceByTenantIdAndYear(tenantId, pattern);
    ...
    public AccountingEntry updateJournalEntry(String tenantId, Long entryId, AccountingEntry entry,
        ...
        if (existingEntry.getEntryStatus() != AccountingEntry.EntryStatus.DRAFT) {
            throw new IllegalStateException(
                    "DRAFT 상태의 분개만 수정할 수 있습니다. 현재 상태: " + existingEntry.getEntryStatus());
        }
        ...
        List<JournalEntryLine> existingLines =
                journalEntryLineRepository.findByJournalEntryId(entryId);
        ...
        log.info("분개 수정 완료: tenantId={}, entryId={}, entryNumber={}", tenantId, entryId,
                savedEntry.getEntryNumber());
