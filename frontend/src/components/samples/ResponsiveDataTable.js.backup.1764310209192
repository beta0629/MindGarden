...
const ResponsiveDataTable = () => {
  ...
  // 모바일 카드 컴포넌트
  const MobileCard = ({ employee, index }) => (
    <div
      key={employee.id}
      style={{
    ...
    >
      ...
      <div >
        ...
        {/* 확장/축소 버튼 */}
        <button
          onClick={() => setExpandedCard(expandedCard === employee.id ? null : employee.id)}
          
        >
          {expandedCard === employee.id ? '▼ 상세 정보 접기' : '▶ 상세 정보 보기'}
        </button>
  ...
  return (
    <>
      ...
      <div 
        className="responsive-data-table-sample" 
      ...
      >
        ...
        >
          <div >
            ...
            >
              {isMobile ? '모바일: 카드형 레이아웃' : '데스크톱/태블릿: 테이블형 레이아웃'} • 정렬, 필터링, 페이지네이션 지원
            </p>
          </div>
          <div 
        ...
        {isMobile ? (
          <div >
            {paginatedData.map((employee, index) => (
              <MobileCard key={employee.id} employee={employee} index={index} />
            ))}
          </div>
        ) : (
          <DesktopTable />
