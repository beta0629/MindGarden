...
## 4. 사용자 시나리오 (확장)
1. 입점사 담당자가 Trinity 홈페이지 하위 온보딩 URL (예: `https://apply.e-trinity.co.kr/onboarding`)에 접속한다.
...
4. **입점사 자신의 PG사 등록** ⭐ (추후 구현)
   - PG사 선택 (TOSS, IAMPORT, KAKAO, NAVER, PAYPAL, STRIPE 등)
   - PG 설정 정보 입력 (API Key, Secret Key, Merchant ID 등)
   - PG 설정 정보 암호화 저장 및 승인 대기
   - 온보딩 요청에 PG 설정 정보 포함
5. 제출하면 백엔드 API에 온보딩 요청 + 결제/구독 정보 + PG 설정 정보가 저장된다.
6. 운영 포털 `/onboarding` 목록에 항목이 나타나며, 운영팀이 심사한다.
7. **PG 설정 승인 후 ERP 자동 구성** ⭐ (추후 구현)
   - PG 설정 승인 완료 시 ERP 자동 구성 트리거
   - 테넌트별 PG 설정 기반 ERP 모듈 자동 활성화
   - 결제/정산 ERP 모듈 자동 연동
   - ERP 초기 데이터 자동 생성 (계정과목, 정산 계좌 등)
8. 승인 결과 및 결제 상태는 추후 이메일/앱 푸시로 통지하는 것을 목표로 한다(Phase 2).

> MindGarden 본 서비스(테넌트/소비자 웹)는 기존 도메인 `m-garden.co.kr` 하위에서 운영되며, 온보딩 완료 후 테넌트별 `{tenant}.m-garden.co.kr` 혹은 `app.m-garden.co.kr`로 연결된다.
...
- **입점사 PG 등록 API** ⭐ (추후 구현)
  - `TenantPgConfigurationService` 활용 (이미 구현됨)
  - PG 설정 정보 암호화 저장
  - PG 설정 승인 프로세스 통합
  - ERP 자동 구성 연계 준비
- **ERP 자동 구성 연계** ⭐ (추후 구현)
...
- 필드:
  ...
  - **입점사 PG사 등록** ⭐ (추후 구현)
    - PG사 선택 (드롭다운)
    - API Key, Secret Key 입력 (암호화 저장)
    - Merchant ID, Store ID 입력 (선택)
    - Webhook URL, Return URL, Cancel URL 입력 (선택)
    - 테스트 모드 선택 (체크박스)
- 안내 문구:
  - “승인까지 최대 3영업일 소요”
