...
public interface UserRepository extends BaseRepository<User, Long> {
     ...
     * PR-A(2026-06-13): branches 테이블 ARCHIVE 후 Long branchId 직접 매핑.
     */
     ...
     *             PR-A(2026-06-13): branches 테이블 ARCHIVE 후 branchName 컬럼 제거,
     *             branchId 만 반환 (Object[] = [branchId, count]).
     */
    @Deprecated
    @Query("SELECT u.branchId, COUNT(u) FROM User u WHERE u.tenantId = :tenantId AND u.isDeleted = false GROUP BY u.branchId ORDER BY u.branchId")
     ...
     */
    @Deprecated
    @Query("SELECT u.branchId, COUNT(u) FROM User u WHERE u.isDeleted = false GROUP BY u.branchId ORDER BY u.branchId")
     ...
     *             PR-A(2026-06-13): branches 테이블 ARCHIVE 후 branchName 컬럼 제거,
     *             branchId 만 반환 (Object[] = [branchId, role, count]).
     */
    @Deprecated
    @Query("SELECT u.branchId, u.role, COUNT(u) FROM User u WHERE u.tenantId = :tenantId AND u.isDeleted = false GROUP BY u.branchId, u.role ORDER BY u.branchId, u.role")
     ...
     */
    @Deprecated
    @Query("SELECT u.branchId, u.role, COUNT(u) FROM User u WHERE u.isDeleted = false GROUP BY u.branchId, u.role ORDER BY u.branchId, u.role")
