...
| 파일 | 역할 | 트리거 요약 | dev/prod 쌍 | reusable | 비고 |
|------|------|-------------|-------------|----------|------|
| `code-quality-check.yml` | 코드 품질 | PR/push `main`·`develop`, `workflow_dispatch`; 스킵: 라벨 `skip-quality` / 커밋 `[skip-quality]`·`[skip ci]` / dispatch `skip_quality` / ops·docs·`.github`·expo·mobile·ops·trinity·md 전용 paths → **동일 job 이름 stub 성공**(required check pending 방지) | — | — | [`code-quality-check.yml`](../../.github/workflows/code-quality-check.yml), [`CI_CODE_QUALITY_AND_MVN_GATE.md`](../guides/testing/CI_CODE_QUALITY_AND_MVN_GATE.md) |
