...
- [DB / 운영 환경변수 SSOT 정책](./DB_ENV_SSOT_POLICY.md) — 운영 MAX 조회 시 source 대상 결정 근거
...
# 3) 운영 (반영 직전)
# 운영 SSOT 는 /etc/mindgarden/prod.env 단일이 목표(점검 문서 부록 D.4 안1, DB_ENV_SSOT_POLICY.md §1).
# 과도기(2026-06-13 기준)에는 DB_HOST/DB_NAME/DB_USERNAME 이 prod-from-dev.env 에 존재하므로
# 두 파일을 함께 source 해야 mysql 접속 변수가 모두 채워진다 (PR-4 머지 후에는 prod.env 단일 source 로 충분).
ssh root@beta74.cafe24.com '
  if [ -f /etc/mindgarden/prod-from-dev.env ]; then source /etc/mindgarden/prod-from-dev.env; fi
