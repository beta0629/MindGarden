...
describe('resolveSessionNumberFromSchedule', () => {
  it('sessionSequence를 sessionNumber보다 우선한다', () => {
    expect(
      resolveSessionNumberFromSchedule({ sessionSequence: 4, sessionNumber: 1 }),
    ).toBe(4);
  });

  it('sessionSequence 없으면 sessionNumber를 사용한다', () => {
    expect(resolveSessionNumberFromSchedule({ sessionNumber: 2 })).toBe(2);
  });

  it('둘 다 없으면 null (기본값 1 금지)', () => {
