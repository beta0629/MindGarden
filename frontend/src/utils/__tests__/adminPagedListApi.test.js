...
import {
  ...
  fetchAdminConsultationMessagesAll
...
describe('adminPagedListApi', () => {
  ...
  test('fetchAdminConsultationMessagesAll always passes page+size', async() => {
    await fetchAdminConsultationMessagesAll({ view: 'admin_ops', page: 2 });
    expect(StandardizedApi.get).toHaveBeenCalledWith(
  ...
  test('fetchAdminConsultationMessagesAll forces defaults when omitted', async() => {
    await fetchAdminConsultationMessagesAll();
    const [, query] = StandardizedApi.get.mock.calls[0];
    expect(query.page).toBe(0);
    expect(query.size).toBe(DEFAULTS.PAGE_SIZE);
  });
});
