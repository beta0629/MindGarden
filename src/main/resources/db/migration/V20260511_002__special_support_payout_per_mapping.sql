-- 특별지원금: 동일 내담자 재매핑(추가 회기권)도 지급되도록 지급 추적을 client→mapping 단위로 변경
-- CalculateSalaryPreview / ProcessIntegratedSalaryCalculation 의 sp 조인·INSERT 반영

ALTER TABLE special_support_monthly_payouts
  ADD COLUMN mapping_id BIGINT NULL COMMENT 'consultant_client_mappings.id' AFTER client_id;

UPDATE special_support_monthly_payouts sp
INNER JOIN (
  SELECT tenant_id, consultant_id, client_id, MIN(id) AS mapping_id
  FROM consultant_client_mappings
  WHERE is_deleted = FALSE
  GROUP BY tenant_id, consultant_id, client_id
) m ON m.tenant_id = sp.tenant_id
   AND m.consultant_id = sp.consultant_id
   AND m.client_id = sp.client_id
SET sp.mapping_id = m.mapping_id
WHERE sp.mapping_id IS NULL;

DELETE FROM special_support_monthly_payouts WHERE mapping_id IS NULL;

ALTER TABLE special_support_monthly_payouts
  MODIFY COLUMN mapping_id BIGINT NOT NULL COMMENT 'consultant_client_mappings.id';

ALTER TABLE special_support_monthly_payouts DROP INDEX uk_ss_payout_tenant_consultant_client_ym;

ALTER TABLE special_support_monthly_payouts
  ADD UNIQUE KEY uk_ss_payout_tenant_consultant_mapping_ym (tenant_id, consultant_id, mapping_id, salary_year_month),
  ADD KEY idx_ss_payout_mapping (mapping_id);
