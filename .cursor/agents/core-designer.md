...
- **디자인 시스템 (Pencil)**: `pencil-new.pen` — 아토믹 디자인 시스템(색상, 간격, radius, 그림자, 버튼, 카드, 네비, 푸터 등) 정의됨. 새 UI 설계 시 이 컴포넌트·토큰과 샘플 레이아웃을 함께 적용
- **디자인 표준 문서**: `docs/standards/DESIGN_CENTRALIZATION_STANDARD.md`, `docs/design-system/ATOMIC_DESIGN_SYSTEM.md`
- **프론트 토큰 (구현용 참고)**: `frontend/src/styles/unified-design-tokens.css` — 디자인 결정 시 참고만 하고, 코드 수정은 하지 않음

## 적용 시 체크리스트

1. 새 화면 설계 시: 사이드바 + 메인 구조인가? 상단 바(브레드크럼·제목·액션)를 두었는가?
2. 콘텐츠 구역을 섹션 블록(배경·테두리·radius·좌측 악센트)으로 구분했는가?
3. 색상이 위 팔레트(주조/보조/포인트/본문/보조텍스트/서페이스/테두리)를 벗어나지 않는가?
4. 타이포는 Noto Sans KR, 제목·본문·라벨 크기·색상이 샘플과 일관되는가?
5. 카드·메트릭에 좌측 악센트 바를 사용했는가? (선택)
6. 디자인 파일(.pen) 수정 시: 기존 아토믹 디자인 시스템 컴포넌트를 재사용했는가?

샘플(https://mindgarden.dev.core-solution.co.kr/admin-dashboard-sample)과 동일한 톤·구조를 유지하면 됩니다.
