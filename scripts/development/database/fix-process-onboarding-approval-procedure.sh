#!/bin/bash
# ProcessOnboardingApproval 프로시저 강제 재생성 스크립트
# 목적: Flyway가 DELIMITER를 제대로 처리하지 못하여 프로시저 정의가 NULL인 문제 해결
# 사용법: 서버에서 직접 실행

set -e

echo "=========================================="
echo "ProcessOnboardingApproval 프로시저 강제 재생성"
echo "=========================================="
echo ""
...
# 프로시저 재생성 SQL 실행
echo "프로시저 재생성 중..."
mysql -h "$DB_HOST" -u "$DB_USERNAME" -p"$DB_PASSWORD" "$DB_NAME" << 'EOFSQL'
-- ProcessOnboardingApproval 프로시저 강제 재생성
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "프로시저 정의 확인:"
    mysql -h "$DB_HOST" -u "$DB_USERNAME" -p"$DB_PASSWORD" "$DB_NAME" -e "SHOW CREATE PROCEDURE ProcessOnboardingApproval\G" | grep -A 1 "Create Procedure"
else
    echo "❌ 프로시저 재생성 실패"
    exit 1
fi
