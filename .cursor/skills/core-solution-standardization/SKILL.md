...
## 4. 데이터 정본 vs 임시 소스 (기관연계·가예약)

장기 정본은 **데이터**다. 잘못된 타입을 소스로 계속 수용·우회하지 않는다. 한 장 요약: `docs/project-management/INSTITUTION_LINK_DATA_SSOT_POLICY.md`. 배포 시 **덮어쓰기 금지**와 함께 동작 (`/core-solution-deployment`).

1. **장기 정본은 데이터.** 타기관 내담자(예: 최가을)는 매핑·일정이 처음부터/또는 보정 후 `INSTITUTION_LINK`(기관연계)여야 한다. `SAME_DAY_CARD`(가예약)로 남은 기존 데이터는 **데이터 수정**(마이그·운영 보정)으로 맞춘다.
2. **소스로 가예약을 기관처럼 취급하는 분기/우회는 임시 정책.** 데이터 정합 후 제거 후보. 임시 우회를 늘리며 데이터 오류를 덮지 말 것.
3. **신규 기능·배포:** 잘못된 타입의 “기존 데이터”를 코드로 계속 수용하기보다, 마이그/운영 보정으로 타입을 바로잡는다. (기능 브랜치 단독 빌드 **덮어쓰기 배포 금지**와 함께 동작)
4. **가예약은 가예약만, 기관연계는 기관연계만** (배정 배타) — 데이터 타입과 UI가 일치해야 함. 교차 배정·교차 취급 금지.
5. **회기권 rem SSOT / 승계이력과 섞지 말 것.** `remainingSessions` 표시·잔여 계산과 session-transfer-history는 분리한다.
