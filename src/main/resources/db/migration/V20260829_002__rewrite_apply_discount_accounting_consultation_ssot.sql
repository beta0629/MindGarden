-- =============================================================================
-- V20260829_002 — ApplyDiscountAccounting INSERT category SSOT (상담료)
--
-- 배경:
--   • ApplyDiscountAccounting 매출 INSERT 가 category='CONSULTATION' 을 기록해
--     financial_transactions 상담료 SSOT(상담료) 와 드리프트.
--   • V20260829_001 이 기존 행을 백필하지만, SP 재배포 없이는 신규 할인 적용이
--     다시 CONSULTATION 을 씀.
--
-- 변경:
--   • DROP + CREATE ApplyDiscountAccounting 만 (표준화 본 기준).
--   • INSERT category 만 'CONSULTATION' → '상담료'. 시그니처·기타 로직 동일.
--   • MySQL 8: fin_proc 라벨 + LEAVE fin_proc (V20260531_004 패턴).
--
-- 금지:
--   • ProcessRefundWithSessionAdjustment / ProcessPartialRefund 재생성 금지
--     (V20260601_001 로 DROP됨).
--   • GetCategoryFinancialBreakdown 등 GROUP BY only SP 수정 금지.
-- =============================================================================
...
fin_proc: BEGIN
    ...
    IF v_existing_count > 0 THEN
        ...
        LEAVE fin_proc;
    ELSE
        -- 4. 매출 거래 생성 (테넌트 격리) — category SSOT: 상담료
