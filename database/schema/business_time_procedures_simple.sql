-- =====================================================
-- 업무 시간 및 정책 관리 PL/SQL 프로시저 (간소화 버전)
-- =====================================================
    ...
    SELECT 
        ...
        CASE 
            ...
            WHEN code_value = 'SLOT_INTERVAL' THEN
                SUBSTRING(code_label, LOCATE('(', code_label) + 1, LOCATE('분', code_label) - LOCATE('(', code_label) - 1)
    ...
    SELECT 
        ...
        CASE 
            WHEN code_value = 'MIN_NOTICE_HOURS' THEN
                SUBSTRING(code_label, LOCATE('(', code_label) + 1, LOCATE('시간', code_label) - LOCATE('(', code_label) - 1)
            WHEN code_value = 'MAX_ADVANCE_DAYS' THEN
                SUBSTRING(code_label, LOCATE('(', code_label) + 1, LOCATE('일', code_label) - LOCATE('(', code_label) - 1)
...
BEGIN
    ...
    DECLARE v_lunch_start_time TIME DEFAULT '12:00:00';
    DECLARE v_lunch_end_time TIME DEFAULT '13:00:00';
    ...
    SELECT 
        MAX(CASE WHEN code_value = 'MIN_NOTICE_HOURS' THEN 
            CAST(SUBSTRING(code_label, LOCATE('(', code_label) + 1, LOCATE('시간', code_label) - LOCATE('(', code_label) - 1) AS UNSIGNED)
        END),
        MAX(CASE WHEN code_value = 'MAX_ADVANCE_DAYS' THEN 
            CAST(SUBSTRING(code_label, LOCATE('(', code_label) + 1, LOCATE('일', code_label) - LOCATE('(', code_label) - 1) AS UNSIGNED)
        END)
