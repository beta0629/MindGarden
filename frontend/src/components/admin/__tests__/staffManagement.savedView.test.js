/**
 * Seq 28g Phase 6 — Staff saved view named views UI
 */
...
const PAGE_ID = USER_MANAGEMENT_SAVED_VIEW_PAGE_IDS.staff;
...
describe('Staff saved view named views (28g-p6)', () => {
  ...
  it('pageId storageKey가 admin.user-management.staff를 사용한다', () => {
  ...
  it('named view 저장·복원 시 filters·viewMode를 유지한다', () => {
    ...
    const storedFilters = { role: 'ADMIN' };

    let viewId;
    act(() => {
      viewId = result.current.saveNamedView('관리 직원', {
        viewMode: 'smallCard',
    ...
    expect(result.current.savedView.viewMode).toBe('smallCard');
