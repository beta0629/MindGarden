-- =====================================================
-- 급여 승인 및 ERP 동기화 프로시저 (표준화 버전)
-- =====================================================
DELIMITER //

DROP PROCEDURE IF EXISTS ApproveSalaryWithErpSync //
...
BEGIN
    ...
    BEGIN
        ...
        SET p_success = FALSE;
        SET p_message = CONCAT('급여 승인 중 오류 발생: ', v_error_message);
    END;
    ...
    SELECT consultant_id, gross_salary, net_salary
    INTO v_consultant_id, v_gross_salary, v_net_salary
    ...
    ELSE
        ...
        SET p_success = TRUE;
        SET p_message = CONCAT('급여 승인 및 ERP 동기화가 완료되었습니다. ERP 동기화 ID: ', v_erp_sync_id);
