...
- **운영 반영 전 하드코딩**: CSS/스타일에서 **`#hex`, `rgb(a)` 리터럴, 검색·CI에 잡히는 고정 색**은 **전부** `var(--mg-*)` 등 토큰으로 치환한다. `/core-solution-standardization`·`ADMIN_LNB_LAYOUT_UNIFICATION_MEETING_HANDOFF.md` **§17** 준수. **core-coder**가 스캔 결과를 소진할 때까지 반복.
