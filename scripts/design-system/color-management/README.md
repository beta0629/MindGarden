...
| `check-token-ssot.js` | **T-D 가드**. codemod 매핑 토큰 ↔ `unified-design-tokens.css` 정의 cross-check (라이트·다크 양쪽). codemod 진입 시 자동 호출. | ERROR / alias 충돌 시 abort |
...
- 본 스크립트와 `check-token-ssot.js` 는 **독립적** 관심사:
