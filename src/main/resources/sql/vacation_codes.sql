-- 휴가 관련 코드 그룹 및 코드값 추가

-- 휴가 타입 코드 그룹 추가
INSERT INTO code_groups (code, name, description, is_active, is_deleted, sort_order, created_at, updated_at, version) 
VALUES ('VACATION_TYPE', '휴가 유형', '상담사 휴가 유형을 나타내는 코드', true, false, 8, NOW(), NOW(), 0);

-- 휴가 상태 코드 그룹 추가  
INSERT INTO code_groups (code, name, description, is_active, is_deleted, sort_order, created_at, updated_at, version)
VALUES ('VACATION_STATUS', '휴가 상태', '휴가 승인 상태를 나타내는 코드', true, false, 9, NOW(), NOW(), 0);
