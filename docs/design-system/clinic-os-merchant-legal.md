...
- Login footer: **minimal `/legal/terms` · `/legal/privacy` · `/legal/products` · `/legal/refund` links** — no product table, no long terms/privacy body.
...
- 환불·취소·청약철회: 공개 crawl `/legal/refund`. 센터 `refundPolicyText` 우선, 없으면 플랫폼 SSOT `## refund`(전상법 수준: 기간·불가 사유·절차·문의).
- 공개 crawl: `/legal/terms` · `/legal/privacy` · `/legal/products` · `/legal/refund` (GET). Modal-only public legal **금지**.
- Footer molecule (`MerchantLegalFooterPreview`): 안내 컬럼 4 Links → `/legal/*`. Account privacy → `/legal/privacy` (not `/privacy`).
