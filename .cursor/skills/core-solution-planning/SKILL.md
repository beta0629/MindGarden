...
이 스킬은 **기획에이전트(core-planner)** 및 “전반 기획이 필요한 작업”에 적용합니다. **기획 에이전트는 오케스트레이터**입니다: 사용자 명령 → 서브에이전트 담당 배정(분배실행) → 각 서브에이전트 실행(의존성 없으면 병렬) → 결과를 기획에게 보고 → 기획이 취합해 **사용자에게 최종 보고**. 병렬로 많은 일을 한 번에 진행할 수 있습니다.
...
- [ ] **분배실행(역할별 실행 분배)** 표가 포함되어 있는가? (디자인→core-designer, 코더→core-coder, 디버그→core-debugger, 쉘→shell, 문서정리→generalPurpose+documentation 등)
