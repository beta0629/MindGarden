-- V20251219_001: CreateOrActivateTenant 프로시저에 서브도메인 파라미터 추가 및 로직 수정
-- 목적: ProcessOnboardingApproval 프로시저와의 파라미터 불일치 해결 및 서브도메인 처리 로직 통합
...
CREATE PROCEDURE CreateOrActivateTenant(
    ...
    IN p_subdomain VARCHAR(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci,  -- 옵셔널: 서브도메인 (NULL 가능, V20251212_003에서 추가)
...
BEGIN
    ...
    -- 주의: 관리자 계정 생성 실패도 로그에 기록하되 프로세스는 계속 진행
    ...
    BEGIN
        ...
        -- 관리자 계정 생성 실패는 경고로 처리 (프로세스는 계속 진행)
        ELSEIF v_error_message LIKE '%INSERT INTO users%' 
            ...
            -- 관리자 계정 생성 실패는 경고 메시지에 추가하되 프로세스는 계속 진행
