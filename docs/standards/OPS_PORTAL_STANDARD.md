...
개발·운영 서버에서는 **역할과 포트(및 Nginx 프록시 역할)를 동일하게** 두고, **도메인만** 서브도메인에서 `dev`를 뺀 패턴으로 맞춘다(예: `ops.dev…` ↔ `ops…`). 환경별 보안·자격증명·DB·Secret·TLS는 달라질 수 있다. 서버 간 **역할·포트 미러링**, 배포 워크플로 쌍, “동일 포트” 전제(호스트당 역할 일치)는 [배포 표준](./DEPLOYMENT_STANDARD.md)의 **개발·운영 역할 동형(미러링)** 절을 따른다.
