...
| 항목 | 내용 |
|------|------|
| **사용성** | **Ops만**. LNB로 **현황 / PG 승인 / 테넌트(slot)** 이동. PG = 목록 → **승인 검토** 또는 **거부 검토** → summary Confirm → 확정. |
| **정보 노출** | 현황 = **pending count만** (대시보드 KPI·장부·스케줄 덤프 금지). PG 마스킹·Confirm 게이트는 **#919 유지**. |
...
Ops 포털·메인 FE Ops PG 화면이 sticky top horizontal nav / 레거시 카드 스킨으로 Clinic-OS와 어긋난다. Phase 1은 **ops-only shell + LNB IA + PG 승인 크롬 계약**만 맞춘다. 새 팔레트·B0KlA·Pencil·풀 테넌트·풀 ops 대시보드 **금지**.
...
| Phase 2 full tenant / full ops dashboard | Phase 1 ONLY |
| 장부·급여·통합스케줄 ops 이식 | 별도 페이지 walk |
...
│    items: 현황 · PG 승인 · 테넌트(slot)
└─ Main column
   ├─ Quiet header (페이지별 h1 + optional ghost 1개)
   ├─ (optional) Summary strip — 현황만 pending count; PG는 목록 상단 strip 최소화
...
| # | 라벨 | 경로 (계약) | 상태 |
|---|------|-------------|------|
| 1 | 현황 | ops home / dashboard 자리 — **placeholder** | pending count만 |
| 2 | PG 승인 | Main FE: `/admin/ops/pg-approval` · frontend-ops: `/pg-approval` | 본 Phase chrome |
| 3 | 테넌트 | slot (frontend-ops `/tenants` 등 기존 경로 유지 가능) | **슬롯만** — full tenant UX Phase 2 |
...
| 계층 | 재사용 (새로 발명 금지) |
|------|-------------------------|
| Atoms | `MGButton`, `SafeText` / 표시 경계, `KpiNumeral`(현황 count) |
| Molecules | ConfirmModal / UnifiedModal 게이트(#919), EmptyState |
| Organisms | `DesktopLnb` · `OpsApprovalQuietHeader` / `SalaryQuietHeader` 패턴 · `OpsApprovalSummaryStrip` 패턴(현황 1셀 축소 가능) |
| Template | `ErpPageShell` · `AdminCommonLayout` + `ContentArea` (main FE) · ops RootLayout 재구성 |

캡슐화: **Shell / LNB IA / PG stage chrome** 경계를 나누고, PG API·마스킹은 기존 모듈에 둔다.
...
| 가맹 ID 마스킹 · center/PG display helpers | LNB 1차: 현황 / PG 승인 / 테넌트 slot |
...
- [ ] LNB 1차 3항만: 현황(placeholder·pending count) · PG 승인 · 테넌트(slot)
...
- Constants: `frontend/src/constants/pgApproval.js`, `frontend-ops/src/constants/pgApproval.ts`
