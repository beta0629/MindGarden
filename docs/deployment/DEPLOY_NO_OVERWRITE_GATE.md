...
**상태**: 필수 (PROD / SSH / Actions 컷오버·머지 전)  
...
**부분 tip 단독 배포·머지 금지.** 심볼 게이트 **6항 전부 PASS**한 **통합 tip 한 번**만 FE+JAR를 올린다. **DATAFIX 0.**
...
통합 tip(IL SSOT + 카드 + 가예약 일지 + OPEN 점유 드래그 + 이관 히스토리 + prepaid 비표시)이 **6항 PASS**한 뒤에만 FE(`/var/www/mindgarden/frontend`) + JAR를 **한 번** 반영한다.  
