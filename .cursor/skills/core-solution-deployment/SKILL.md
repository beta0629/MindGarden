...
- **데이터 정본 vs 임시 소스**: 장기 정본은 **데이터**. 잘못된 타입(`SAME_DAY_CARD`로 남은 기관연계 등)은 코드 우회로 덮지 말고 마이그/운영 보정으로 `INSTITUTION_LINK` 등에 맞춘다. 가예약↔기관연계 교차 취급·rem SSOT/승계이력 혼용 금지. 상세: `/core-solution-standardization` 「데이터 정본 vs 임시 소스」, `docs/project-management/INSTITUTION_LINK_DATA_SSOT_POLICY.md`. **덮어쓰기 배포 금지**와 함께 동작.
