...
**휴대폰으로 촬영한 4K 동영상도 업로드 가능합니다!**

- **자동 리사이징**: 4K 동영상(3840x2160)은 서버에서 자동으로 1080p(1920x1080)로 변환됩니다
- **파일 크기 제한**: 최대 500MB까지 업로드 가능
- **형식**: 모든 비디오 형식 지원 (MP4, MOV, AVI 등)
- **처리 시간**: 리사이징에 따라 업로드 시간이 다소 소요될 수 있습니다

> **참고**: 서버에 FFmpeg가 설치되어 있지 않은 경우, 원본 파일이 그대로 저장됩니다. 최적의 성능을 위해 서버에 FFmpeg 설치를 권장합니다.
...
- **최소**: 1280x720 (HD)
- **최대**: 3840x2160 (4K 지원, 자동으로 1080p로 리사이징됨)
- **4K 동영상**: 휴대폰으로 촬영한 4K 동영상도 업로드 가능하며, 서버에서 자동으로 1080p로 변환됩니다
...
- **권장**: 10MB 이하
- **최대**: 500MB (4K 동영상 지원)
- **자동 리사이징**: 4K 동영상은 자동으로 1080p로 변환됩니다
