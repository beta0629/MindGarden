...
public class PlSqlStatisticsServiceImpl implements PlSqlStatisticsService {
    ...
    public String updateDailyStatistics(String branchCode, LocalDate statDate) {
        ...
        try {
            ...
            log.info("✅ 일별 통계 PL/SQL 프로시저 실행 완료: tenantId={}, statDate={}, success={}", tenantId,
                    statDate, success);
            return success ? "SUCCESS: " + message : "ERROR: " + message;

        } catch (Exception e) {
            log.error("❌ 일별 통계 PL/SQL 프로시저 실행 실패: tenantId={}, statDate={}, 오류={}", tenantId,
    ...
    public String updateConsultantPerformance(Long consultantId, LocalDate performanceDate) {
        log.info("📈 상담사 성과 PL/SQL 프로시저 호출: consultantId={}, performanceDate={}", consultantId,
                performanceDate);
    ...
    public int performDailyPerformanceMonitoring(LocalDate monitoringDate) {
        ...
        try {
            ...
            log.error("❌ 일일 성과 모니터링 PL/SQL 프로시저 실행 실패: monitoringDate={}, 오류={}", monitoringDate,
                    e.getMessage(), e);
            return 0;
        }
    }
    ...
    /**
     * 통계 일관성 검증 표준화 2025-12-06: branchCode 파라미터는 레거시 호환용으로 유지되지만 사용하지 않음
     */
    @Override
    public boolean validateStatisticsConsistency(String branchCode, LocalDate statDate) {
        ...
        try {
            ...
            if (javaResults.isEmpty() || plsqlResults.isEmpty()) {
                log.warn("⚠️ 비교할 통계 데이터가 없습니다: javaResults={}, plsqlResults={}", javaResults.size(),
                        plsqlResults.size());
                return false;
            }
            ...
            boolean consistent = compareValues(javaData.get("total_consultations"),
                    plsqlData.get("total_consultations"))
                    && compareValues(javaData.get("completed_consultations"),
                            plsqlData.get("completed_consultations"))
                    && compareValues(javaData.get("cancelled_consultations"),
                            plsqlData.get("cancelled_consultations"))
                    && compareValues(javaData.get("total_revenue"), plsqlData.get("total_revenue"));

            log.info("🔍 통계 일관성 검증 완료: branchCode={}, statDate={}, 일관성={}", branchCode, statDate,
                    consistent);
            ...
            log.error("❌ 통계 일관성 검증 실패: branchCode={}, statDate={}, 오류={}", branchCode, statDate,
                    e.getMessage(), e);
            return false;
        }
    }
