...
test.describe('내담자 마이페이지 화면 스모크', () => {
  ...
  test('/client/mypage 에서 페이지 testid가 보인다', async ({
    page,
  }: {
    page: Page;
  }) => {
    await page.goto('/client/mypage', { waitUntil: 'domcontentloaded' });
    await expect(page).toHaveURL(/\/client\/mypage/, { timeout: 15_000 });

    const pageRoot = page.getByTestId('client-mypage-page');
