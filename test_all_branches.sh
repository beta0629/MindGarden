...
# bash 4.0+ 연관 배열 지원 확인
if [ "${BASH_VERSION%%.*}" -lt 4 ]; then
    echo "이 스크립트는 bash 4.0 이상이 필요합니다."
    echo "현재 bash 버전: $BASH_VERSION"
    exit 1
fi
...
# 지점별 테스트 계정 정보
declare -A BRANCH_ACCOUNTS=(
    ["HQ"]="super_hq_admin@mindgarden.com:admin123"
    ["MAIN001"]="superadmin@mindgarden.com:admin123"
    ["GANGNAM"]="gangnam_admin@mindgarden.com:admin123"
    ["HONGDAE"]="hongdae_admin@mindgarden.com:admin123"
    ["JAMSIL"]="jamsil_admin@mindgarden.com:admin123"
    ["SINCHON"]="sinchon_admin@mindgarden.com:admin123"
    ["BUSAN"]="busan_admin@mindgarden.com:admin123"
    ["DAEGU"]="daegu_admin@mindgarden.com:admin123"
    ["INCHEON"]="incheon_admin@mindgarden.com:admin123"
    ["GWANGJU"]="gwangju_admin@mindgarden.com:admin123"
)
...
test_basic_apis() {
    ...
    # 결과 저장
    TEST_RESULTS["$branch_code-$test_name"]="$passed/$total"
...
test_branch() {
    local branch_code=$1
    local account_info=${BRANCH_ACCOUNTS[$branch_code]}
    
    if [ -z "$account_info" ]; then
        log_error "지점 계정 정보 없음: $branch_code"
        return 1
    fi
    
    IFS=':' read -r email password <<< "$account_info"
    ...
    local branch_total=0
    
    # 기본 API 테스트
    if test_basic_apis "$branch_code"; then
        branch_passed=$((branch_passed + 1))
    fi
    branch_total=$((branch_total + 1))
    
    # ERP 대시보드 테스트
    if test_erp_dashboard "$branch_code"; then
        branch_passed=$((branch_passed + 1))
    fi
    branch_total=$((branch_total + 1))
    
    # 환불 시스템 테스트
    if test_refund_system "$branch_code"; then
        branch_passed=$((branch_passed + 1))
    fi
    branch_total=$((branch_total + 1))
    ...
    # 결과 저장
    TEST_RESULTS["$branch_code-전체"]="$branch_passed/$branch_total"
    
    log_info "=========================================="
    log_info "지점 테스트 완료: $branch_code ($branch_passed/$branch_total 통과)"
    log_info "=========================================="
...
main() {
    ...
    # 각 지점별 테스트 실행
    for branch_code in "${!BRANCH_ACCOUNTS[@]}"; do
        total_branches=$((total_branches + 1))
        
        if test_branch "$branch_code"; then
            passed_branches=$((passed_branches + 1))
        fi
        
        echo "" >> $RESULTS_FILE
        echo "지점: $branch_code" >> $RESULTS_FILE
        echo "전체 결과: ${TEST_RESULTS["$branch_code-전체"]}" >> $RESULTS_FILE
        echo "기본 API: ${TEST_RESULTS["$branch_code-기본 API 테스트"]}" >> $RESULTS_FILE
        echo "ERP 대시보드: ${TEST_RESULTS["$branch_code-ERP 대시보드 테스트"]}" >> $RESULTS_FILE
        echo "환불 시스템: ${TEST_RESULTS["$branch_code-환불 시스템 테스트"]}" >> $RESULTS_FILE
        echo "----------------------------------------" >> $RESULTS_FILE
    done
