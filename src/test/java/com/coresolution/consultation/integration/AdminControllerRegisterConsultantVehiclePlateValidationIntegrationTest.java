...
/**
 * POST /api/v1/admin/consultants — {@code vehiclePlate} Bean Validation 실패 시
 * HTTP 400 및 ErrorResponse 필드 검증 (MockMvc).
 * 내담자 {@link AdminControllerRegisterClientVehiclePlateValidationIntegrationTest} 와 대칭.
 *
 * @author CoreSolution
 * @since 2026-09-04
 */
...
@DisplayName("AdminController 상담사 등록 vehiclePlate 검증 MockMvc 통합 테스트")
class AdminControllerRegisterConsultantVehiclePlateValidationIntegrationTest {
    ...
    private User adminUser() {
        User user = new User();
        user.setId(1L);
        user.setUserId("admin-consultant-plate-mvc");
        user.setEmail("admin-consultant-plate-mvc@test.com");
        user.setName("테스트관리자");
    ...
    private Map<String, Object> consultantBodyWithPlate(String vehiclePlate) {
        Map<String, Object> body = new HashMap<>();
        body.put("email", "mvc-consultant-plate-" + UUID.randomUUID() + "@test.com");
    ...
    void registerConsultant_invalidPlateCharacter_returns400WithDetails() throws Exception {
        mockMvc.perform(post("/api/v1/admin/consultants")
                        ...
                        .content(objectMapper.writeValueAsString(consultantBodyWithPlate("12@3456"))))
    ...
    void registerConsultant_plateTooLong_returns400WithDetails() throws Exception {
        String tooLong = "1".repeat(33);
        mockMvc.perform(post("/api/v1/admin/consultants")
                        .sessionAttr(SessionConstants.USER_OBJECT, adminUser())
                        .sessionAttr(SessionConstants.TENANT_ID, TEST_TENANT_ID)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(consultantBodyWithPlate(tooLong))))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.status").value(400))
                .andExpect(jsonPath("$.errorCode").value(BEAN_VALIDATION_CODE))
