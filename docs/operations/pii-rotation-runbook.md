...
| `PII_ROTATION_ADMIN_TOKEN` | env | HQ_MASTER 권한 JWT (본 워크플로 admin endpoint 호출용, 회전 직전 발급) |
...
본 워크플로는 `secrets.PII_ROTATION_ADMIN_TOKEN` (HQ_MASTER 권한 JWT) 으로 admin endpoint 를 호출한다.

### 5.1 발급 원칙

- **권한**: HQ_MASTER 권한 사용자 1명 (운영팀 책임자)
...
1. **HQ_MASTER 계정으로 BE 로그인**:
   ...
   curl -sS -X POST "${LOGIN_URL}" \
     ...
     -d '{"username":"<HQ_MASTER_USERNAME>","password":"<HQ_MASTER_PASSWORD>"}' \
...
- HQ_MASTER 계정으로 발급한 토큰을 본 회전 외 목적으로 재사용 금지
...
| 403 | HQ_MASTER 권한 없음 | 사용자 권한 확인 → 적절한 HQ_MASTER 계정으로 재발급 |
