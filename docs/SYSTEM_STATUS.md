...
### **프리랜서 세금 시스템 개선 및 사업자 등록 기능** 🏢
- **사업자 등록 시스템 구현**
  - ConsultantSalaryProfile 엔티티에 사업자 등록번호, 사업자명 필드 추가
  - Hibernate 자동 테이블 스키마 업데이트
  - 사업자 등록 여부에 따른 세금 계산 로직 차별화
...
### **11. 프랜차이즈 지점 관리 시스템** 🏪
- **지점 엔티티**: 완전한 지점 정보 관리 (코드, 이름, 유형, 상태, 주소, 연락처)
- **지점 유형**: 본점, 가맹점, 직영점, 제휴점 지원
- **지점 상태**: 계획중, 준비중, 운영중, 일시정지, 폐점 상태 관리
- **계층 구조**: 상위/하위 지점 관계 및 전체 계층 구조 조회
- **지점장 관리**: 지점장 지정/해제 기능
- **운영 정보**: 운영 시간, 휴무일, 최대 수용 인원 관리
- **지점별 설정**: JSON 형태 개별 설정 및 커스터마이징
- **권한 기반 접근**: 역할별 지점 데이터 접근 제어
- **검색 및 필터링**: 지점명, 코드, 유형, 상태별 검색

**주요 컴포넌트**:
- `Branch.java`: 지점 엔티티 (유형, 상태 enum 포함)
- `BranchService.java`: 지점 비즈니스 로직 인터페이스
- `BranchServiceImpl.java`: 지점 서비스 구현체
- `BranchController.java`: 지점 관리 REST API 컨트롤러
- `BranchRepository.java`: 지점 데이터 접근 계층
- `BranchManagement.js`: 지점 관리 프론트엔드 컴포넌트

### **12. 사용자 역할 확장 시스템** 👥
- **다중 역할 시스템**: CLIENT, CONSULTANT, ADMIN, BRANCH_SUPER_ADMIN, HQ_ADMIN, SUPER_HQ_ADMIN, HQ_MASTER
- **지점별 권한 관리**: 지점별 데이터 격리 및 접근 제어
- **계층적 권한 구조**: 본사 → 지점 → 개별 사용자 권한 체계
- **동적 권한 할당**: 역할별 메뉴 및 기능 동적 제어
- **지점장 시스템**: 지점별 관리자 권한 및 책임 분리
- **보안 격리**: 지점 간 데이터 접근 제한 및 보안 강화
- **권한 변경 이력**: 모든 권한 변경 사항 추적 및 기록
- **사용자 이동**: 지점 간 사용자 이전 및 데이터 마이그레이션

**주요 컴포넌트**:
- `UserRole.java`: 확장된 사용자 역할 enum
- `DynamicPermissionService.java`: 동적 권한 관리 서비스
- `USER_ROLE_SYSTEM.md`: 상세 권한 체계 문서
- `permission_codes.sql`: 권한 관련 공통 코드

### **13. 통합 결제 시스템** 💳
- **토스페이먼츠 API 연동**: 카드, 계좌이체, 가상계좌 등 다양한 결제 방법 지원
- **아임포트 연동**: 다중 PG사 지원을 위한 백업 연동
- **실시간 Webhook 처리**: 결제 상태 변경 실시간 알림 및 처리
- **결제 보안**: HMAC-SHA256 서명 검증, 타임스탬프 유효성 검사
- **자동 상태 관리**: 결제 완료, 실패, 취소, 환불 상태 자동 처리
- **재무 시스템 연동**: 결제 승인 시 자동 수입 거래 생성
- **만료 처리**: 스케줄러를 통한 만료된 결제 자동 처리
- **결제 통계**: 일/월/년 단위 결제 현황 및 분석

**주요 컴포넌트**:
- `Payment.java`: 결제 엔티티 (상태, 방법, 제공자 enum 포함)
- `PaymentService.java`: 결제 비즈니스 로직 인터페이스
- `PaymentServiceImpl.java`: 결제 서비스 구현체
- `TossPaymentServiceImpl.java`: 토스페이먼츠 전용 구현체
- `PaymentController.java`: 결제 REST API 컨트롤러
- `PaymentStatusService.java`: 결제 상태 관리 서비스
