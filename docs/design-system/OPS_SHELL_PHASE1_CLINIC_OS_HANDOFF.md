...
**테넌트 본문 (Phase 1)**: `docs/design-system/OPS_TENANTS_CORE_CLINIC_OS_HANDOFF.md` — **LNB slot + EmptyState twin (empty/coming)** · 풀 관리 = Deferred  
...
**권위**: Leader Phase 1 축소 — **Phase 1 = ops shell + PG only**.  
...
| 항목 | 내용 |
|------|------|
| **사용성** | **Ops만**. LNB로 **테넌트(slot · empty/coming) / PG 승인 / 현황(summary)** 이동. 테넌트 클릭 ≠ 풀 관리. PG = 목록 → **승인 검토** 또는 **거부 검토** → summary Confirm → 확정. |
| **정보 노출** | 현황 = **pending count만** (대시보드 KPI·장부·스케줄 덤프 금지). PG 마스킹·Confirm 게이트는 **#919 유지**. 테넌트 목록·count·들어가기·정지/재개 **Phase 1 비노출** (tenants empty 핸드오프). |
...
Ops 포털·메인 FE Ops PG 화면이 sticky top horizontal nav / 레거시 카드 스킨으로 Clinic-OS와 어긋난다. Phase 1은 **ops-only shell + LNB IA + PG 승인 크롬**을 Must-ship으로 두고, **테넌트는 LNB slot + Clinic-OS 품질 empty/coming**만 둔다(상세: tenants 핸드오프). 카드·strip3·정지/재개 풀 관리는 **Phase 1.5/2 Deferred**. 새 팔레트·B0KlA·Pencil·풀 ops 대시보드(장부·스케줄·급여 이식) **금지**.

**LNB 순서 (Must-ship)**: 확정 순서 = **테넌트 · PG 승인 · 현황**. 테넌트 = **slot** (empty 본문) — 풀 관리 main body로 상향하지 않음.
...
| 구어체 카피 (예: 「손볼」) | 금지 |
| 테넌트 풀 관리 (strip3 · cards · 들어가기 · 정지/재개) | Phase 1 OUT · Deferred — rushed stub FAIL |
...
### 3.2 테넌트 (LNB slot · empty/coming)

```
Quiet header — h1「테넌트」· topbar product「ops · 테넌트 격리」
└─ stage → EmptyState twin only
...
| # | 라벨 | 경로 (계약) | 상태 |
|---|------|-------------|------|
| 1 | **테넌트** | frontend-ops `/tenants` 등 | **slot** — empty/coming (`OPS_TENANTS_CORE_CLINIC_OS_HANDOFF.md`) · 풀 관리 Deferred |
| 2 | PG 승인 | Main FE: `/admin/ops/pg-approval` · frontend-ops: `/pg-approval` | 본 Phase chrome (**main**) |
| 3 | 현황 | ops home / dashboard 자리 | **summary** — pending count만 |

**폐기**: 테넌트 = main body cards Must-ship (구 Leader 상향안). 구현 IA 상수·LNB 렌더·본문은 본 표·tenants empty 계약에 맞출 것.
...
| 테넌트 empty | EmptyState twin · primary CTA 없음 | tenants Phase 1 |
...
| 테넌트 | 동일 empty/coming 계약 | 상세: `OPS_TENANTS_CORE_CLINIC_OS_HANDOFF.md` |
...
| Organisms | `DesktopLnb` · `OpsApprovalQuietHeader` / `SalaryQuietHeader` 패턴 · `OpsApprovalSummaryStrip`(현황/PG용) — TenantCenterCard **Deferred** |
| Template | `ErpPageShell` · `AdminCommonLayout` + `ContentArea` (main FE) · ops RootLayout 재구성 |

캡슐화: **Shell / LNB IA / PG stage chrome / Tenant empty slot** 경계를 나누고, PG API·마스킹은 기존 모듈에 둔다.
---
...
| 가맹 ID 마스킹 · center/PG display helpers | LNB 1차: **테넌트(slot) / PG 승인 / 현황(summary)** |
...
| | 테넌트 empty/coming → `OPS_TENANTS_CORE_CLINIC_OS_HANDOFF.md` |
...
| 테넌트 empty/coming/Forbidden | tenants handoff §4 · §8 |
...
- [ ] LNB 1차 3항만 · **순서: 테넌트(slot) · PG 승인 · 현황(summary)**
- [ ] 테넌트 본문: `OPS_TENANTS_CORE_CLINIC_OS_HANDOFF.md` Phase 1 empty/coming 준수 (strip3·cards·들어가기·정지/재개 OUT)
...
- `docs/design-system/OPS_TENANTS_CORE_CLINIC_OS_HANDOFF.md` (**테넌트 slot · empty/coming · 풀 관리 Deferred**)
