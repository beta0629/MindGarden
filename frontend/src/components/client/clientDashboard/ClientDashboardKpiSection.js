/**
 * Client Dashboard — KPI 4-grid 섹션 (v1.4)
 *
 * @author CoreSolution
 * @since 2026-07-07
 */

import React from 'react';
import PropTypes from 'prop-types';
import { Calendar, Heart, Bell, CheckCircle } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { useNavigate } from 'react-router-dom';
import { ContentKpiRow } from '../../dashboard-v2/content';
import { CLIENT_DASHBOARD_KPI_ROUTES } from '../../../constants/clientDashboardRoutes';
import { toSafeNumber } from '../../../utils/safeDisplay';
import ClientDashboardSectionBlock from './ClientDashboardSectionBlock';
import {
  CLIENT_DASHBOARD_KPI_SECTION_TEST_ID,
  CLIENT_KPI_COMPLETED_LABEL,
  CLIENT_KPI_SECTION_DESC,
  CLIENT_KPI_SECTION_TITLE
} from './constants';

const KPI_ICON_PROPS = { size: 28, strokeWidth: 2, 'aria-hidden': true };
const KPI_ERROR_VALUE = '—';

const ClientDashboardKpiSection = ({
  remainingSessions,
  thisMonthScheduleCount,
  unreadMessageCount,
  completedCount,
  loading = false,
  error = false
}) => {
  const navigate = useNavigate();
  const { t } = useTranslation();
  const unread = toSafeNumber(unreadMessageCount);
  const kpiValue = (value) => (error ? KPI_ERROR_VALUE : toSafeNumber(value));

  return (
    <ClientDashboardSectionBlock
      title={CLIENT_KPI_SECTION_TITLE}
      subtitle={CLIENT_KPI_SECTION_DESC}
      accentVariant="accent"
      dataTestId={CLIENT_DASHBOARD_KPI_SECTION_TEST_ID}
      className="client-dashboard__section client-dashboard__section--kpi"
    >
      <ContentKpiRow
        className="client-dashboard__kpi-row"
        loading={loading}
        items={[
          {
            id: 'remainingSessions',
            icon: <Heart {...KPI_ICON_PROPS} />,
            label: t('common:client.ClientDashboard.t_e9792c10'),
            value: kpiValue(remainingSessions),
            iconVariant: 'gray',
            onClick: () => navigate(CLIENT_DASHBOARD_KPI_ROUTES.REMAINING_SESSIONS)
          },
          {
            id: 'thisMonthConsultations',
            icon: <Calendar {...KPI_ICON_PROPS} />,
            label: t('common:client.ClientDashboard.t_4af64dc5'),
            value: kpiValue(thisMonthScheduleCount),
            iconVariant: 'blue',
            onClick: () => navigate(CLIENT_DASHBOARD_KPI_ROUTES.THIS_MONTH_SESSIONS)
          },
          {
            id: 'unreadMessages',
            icon: <Bell {...KPI_ICON_PROPS} />,
            label: t('common:client.ClientDashboard.t_83cce32e'),
            value: kpiValue(unread),
            iconVariant: 'orange',
            subtitleBadge: !error && unread > 0 ? '새 소식' : null,
            badgeVariant: 'green',
            onClick: () => navigate(CLIENT_DASHBOARD_KPI_ROUTES.UNREAD_MESSAGES)
          },
          {
            id: 'completedConsultations',
            icon: <CheckCircle {...KPI_ICON_PROPS} />,
            label: CLIENT_KPI_COMPLETED_LABEL,
            value: kpiValue(completedCount),
            iconVariant: 'green',
            onClick: () => navigate(CLIENT_DASHBOARD_KPI_ROUTES.COMPLETED_SESSIONS)
          }
        ]}
      />
    </ClientDashboardSectionBlock>
  );
};

ClientDashboardKpiSection.propTypes = {
  remainingSessions: PropTypes.number,
  thisMonthScheduleCount: PropTypes.number,
  unreadMessageCount: PropTypes.number,
  completedCount: PropTypes.number,
  loading: PropTypes.bool,
  error: PropTypes.bool
};

export default ClientDashboardKpiSection;
