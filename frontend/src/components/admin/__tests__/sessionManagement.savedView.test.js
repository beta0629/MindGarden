/**
 * Seq 28g Phase 2 Set D — session-management saved view named views UI
 */
...
const PAGE_ID = SESSION_MANAGEMENT_SAVED_VIEW_PAGE_ID;
const SCOPE = { tenantId: 'tenant-test', userId: 'user-test' };
const DEFAULT_SAVED_VIEW = buildSessionManagementDefaultSavedView(
  SESSION_MANAGEMENT_DEFAULT_VIEW_MODE
);

describe('Session management saved view (28g Phase 2 Set D)', () => {
  ...
  it('pageId storageKey가 admin.session-management를 사용한다', () => {
    const savedViewKey = buildSavedViewStorageKey(SCOPE, PAGE_ID);

    expect(savedViewKey).toContain(PAGE_ID);
  });

  it('named view 저장·복원 시 filters·viewMode·activeTab을 유지한다', () => {
    ...
    let viewId;
    act(() => {
      viewId = result.current.saveNamedView('활성 매핑 탭', {
        viewMode: 'list',
