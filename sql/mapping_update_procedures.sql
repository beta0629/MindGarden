-- 매핑 수정 관련 PL/SQL 프로시저
-- 매핑 정보 수정 시 ERP 시스템과 연동하여 모든 관련 데이터를 일괄 업데이트

DELIMITER //

-- 매핑 정보 수정 프로시저
CREATE OR REPLACE PROCEDURE UpdateMappingInfo(
...
BEGIN
    ...
    ELSE
        ...
        -- 5. ERP 수익 데이터 업데이트 (기존 데이터가 있다면)
        UPDATE erp_revenue_data 
        SET 
            amount = p_new_package_price,
            package_name = p_new_package_name,
            session_count = p_new_total_sessions,
            updated_at = NOW()
        ...
        IF v_price_difference != 0 THEN
            INSERT INTO accounting_adjustments (
                ...
                v_price_difference,
                CONCAT('패키지 가격 변경: ', v_old_package_price, '원 → ', p_new_package_price, '원'),
                NOW(),
                p_updated_by
            );
        END IF;
        ...
        -- 7. 세션 수 변경 로그
        IF v_session_difference != 0 THEN
            INSERT INTO session_adjustments (
                ...
                p_mapping_id,
                'TOTAL_SESSION_CHANGE',
                v_session_difference,
                CONCAT('총 세션 수 변경: ', v_old_total_sessions, '회 → ', p_new_total_sessions, '회'),
                NOW(),
                p_updated_by
            );
        END IF;
...
-- 매핑 통계 업데이트 프로시저
CREATE OR REPLACE PROCEDURE UpdateMappingStatistics(
    IN p_mapping_id BIGINT,
...
BEGIN
    ...
    -- 상담사별 통계 업데이트
    INSERT INTO consultant_statistics (
        consultant_id,
        total_revenue,
        total_sessions,
        used_sessions,
...
-- 매핑 수정 권한 확인 프로시저
CREATE OR REPLACE PROCEDURE CheckMappingUpdatePermission(
    IN p_mapping_id BIGINT,
    IN p_user_id BIGINT,
