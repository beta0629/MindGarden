...
# head closes early → SIGPIPE 141 under pipefail; tolerate it
pipe_ok() { "$@" || { local ec=$?; [[ $ec -eq 141 ]] && return 0; return $ec; }; }
...
# mindgarden + core-solution only (exclude e-trinity/ops/apply noise)
...
for _f in "${NGINX_LOGS[@]+"${NGINX_LOGS[@]}"}"; do
  [[ -f "$_f" ]] || continue
  case "$_f" in *error*) continue ;; esac
...
analyze_day() {
  ...
  echo "--- req/hour histogram ---"
  grep -oE '\[[0-9]{2}/[A-Za-z]{3}/[0-9]{4}:[0-9]{2}' "$tmp" \
    | cut -d: -f2 \
    | sort | uniq -c | sort -k2 \
    | awk '{printf "  %s: %s\n", $2, $1}' || true
  local peak_line peak_req peak_hour
  ...
  echo "--- req/hour last 6h (max hour among TODAY lines, then -5) ---"
  local hours_file maxh start h hh c
  hours_file=$(mktemp)
  grep -oE '\[[0-9]{2}/[A-Za-z]{3}/[0-9]{4}:[0-9]{2}' "$tmp" \
  ...
  if [[ ! -s "$hours_file" ]]; then
    ...
    maxh=$(sort -n "$hours_file" | tail -1 | sed 's/^0*//;s/^$/0/')
    maxh=${maxh:-0}
    # Prefer wall-clock hour in Asia/Seoul when label is TODAY
    if [[ "$label" == "TODAY" ]]; then
      maxh=$(TZ=Asia/Seoul date +%H | sed 's/^0*//;s/^$/0/')
    fi
    start=$((maxh - 5))
