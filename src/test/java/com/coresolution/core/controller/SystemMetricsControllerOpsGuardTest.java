...
/**
 * {@link SystemMetricsController} OPS 가드 회귀 테스트 (Phase 2 — ops-portal-migration).
 *
 * @author CoreSolution
 * @since 2026-06-15
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("SystemMetricsController OPS 가드 회귀 (Phase 2)")
class SystemMetricsControllerOpsGuardTest {
    ...
    void setUp() throws Exception {
        ...
        SystemMetricsController controller = new SystemMetricsController(
            systemMetricRepository, opsTenantConstants);
    ...
    void classLevelPreAuthorize_isOps() {
        PreAuthorize annotation = SystemMetricsController.class.getAnnotation(PreAuthorize.class);
    ...
    @Test
    @DisplayName("[#1] OPS + HQ 테넌트 → /current 정상")
    void hybrid_opsRole_hqTenant_currentOk() throws Exception {
        TenantContextHolder.setTenantId(HQ_TENANT_ID);
        mockMvc.perform(get("/api/v1/monitoring/system-metrics/current"));
    }

    @Test
    @DisplayName("[#2] OPS + 외부 테넌트 → /current AccessDeniedException")
    void hybrid_opsRole_externalTenant_currentBlocked() {
    ...
    @Test
    @DisplayName("[#3] OPS + 외부 테넌트 → /history 차단")
    void hybrid_opsRole_externalTenant_historyBlocked() {
        TenantContextHolder.setTenantId(EXTERNAL_TENANT_ID);
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->
            mockMvc.perform(get("/api/v1/monitoring/system-metrics/history")
                .param("metricType", "CPU_LOAD")
                .param("minutes", "10")))
            .satisfies(e -> {
                Throwable root = e;
                while (root.getCause() != null && root.getCause() != root) {
    ...
    @Test
    @DisplayName("[#4] OPS + 외부 테넌트 → /summary 차단")
    void hybrid_opsRole_externalTenant_summaryBlocked() {
        TenantContextHolder.setTenantId(EXTERNAL_TENANT_ID);
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->
            mockMvc.perform(get("/api/v1/monitoring/system-metrics/summary")
                .param("minutes", "10")))
            .satisfies(e -> {
                Throwable root = e;
                while (root.getCause() != null && root.getCause() != root) {
    ...
    @Test
    @DisplayName("[#5] HQ_MASTER 잔존 표현식 0 — @PreAuthorize 정리 회귀")
    void hybrid_hqMasterExpression_eliminated() throws Exception {
        java.nio.file.Path source = java.nio.file.Path.of(
            "src/main/java/com/coresolution/core/controller/SystemMetricsController.java");
        ...
        assertThat(executableHqMasterCount)
            .as("SystemMetricsController @PreAuthorize 에 HQ_MASTER 가 잔존하면 안 된다 (Javadoc 제외)")
            .isZero();
    }
}
