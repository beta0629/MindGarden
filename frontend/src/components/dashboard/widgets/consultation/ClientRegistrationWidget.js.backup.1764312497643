...
const ClientRegistrationWidget = ({ widget, user }) => {
  ...
  const title = config.title || '내담자 등록';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const requestData = {
        ...
        ...(formData.consultationPurpose && { consultationPurpose: formData.consultationPurpose }),
        ...(formData.consultationHistory && { consultationHistory: formData.consultationHistory }),
      ...
      const response = await apiPost('/api/admin/clients', requestData);
      
      if (response) {
        notificationManager.show('내담자가 성공적으로 등록되었습니다.', 'success');
      ...
      {showForm ? (
        ...
        <div className="widget-body">
          <div className="widget-empty">
            <i className="bi bi-person-plus"></i>
            <p>{config.emptyMessage || '내담자 등록 버튼을 클릭하여 새 내담자를 등록하세요.'}</p>
          </div>
        </div>
      )}
    </div>
