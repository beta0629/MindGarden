...
const RECORD_QUERY_KEYS = {
  ...
  detail: (id: string | number) => [...RECORD_QUERY_KEYS.details(), id] as const,
};
...
export function useConsultationRecords(params: RecordsParams) {
  ...
  return useInfiniteQuery<PaginatedResponse<ConsultationRecord>>({
    ...
    queryFn: ({ pageParam }) =>
      apiGet<PaginatedResponse<ConsultationRecord>>(CONSULTATION_RECORD_API.records(consultantId), {
...
export function usePendingRecords(consultantId: string | number | undefined) {
  return useQuery<PendingRecord[]>({
    ...
    queryFn: () =>
      apiGet<PendingRecord[]>(`${CONSULTATION_RECORD_API.records(consultantId!)}/pending`),
    enabled: !!consultantId,
    staleTime: 1000 * 60 * 2,
  });
}
...
export function useRecordDetail(
  ...
  return useQuery<ConsultationRecord>({
    ...
    queryFn: () => apiGet<ConsultationRecord>(CONSULTATION_RECORD_API.detail(recordId!)),
    enabled: !!recordId,
    staleTime: 1000 * 60 * 5,
    ...options,
  });
...
export function useCreateRecord() {
  ...
  return useMutation({
    mutationFn: (input: CreateRecordInput) =>
      apiPost<ConsultationRecord>(CONSULTATION_RECORD_API.CREATE_RECORD, input),
    onSuccess: () => {
...
export function useUpdateRecord() {
  ...
  return useMutation({
    mutationFn: ({ recordId, ...input }: Partial<CreateRecordInput> & { recordId: number }) =>
      apiPut<ConsultationRecord>(CONSULTATION_RECORD_API.detail(recordId), input),
