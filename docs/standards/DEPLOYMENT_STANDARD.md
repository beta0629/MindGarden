...
- **운영 통합 진입점(단일)**: [`deploy-unified-production.yml`](../../.github/workflows/deploy-unified-production.yml) — `workflow_dispatch`로 Core 운영(`deploy-production`)·Trinity·Ops(프론트·백엔드)·표준 프로시저 운영을 **순차 디스패치 후 `gh run watch`로 감시**한다. 단계별 on/off는 워크플로 입력으로 선택한다. 온보딩 백엔드 전용 운영 워크플로는 없으므로 팀 절차로 보완한다.
...
**운영 MySQL에 표준 프로시저만 적용**할 때는 `deploy-procedures-production-mysql.yml`을 수동 실행한다(`workflow_dispatch`). 입력 `confirm`에 정확히 `CONFIRM`을 넣어야 하며, 필수 시크릿은 `PRODUCTION_HOST`, `PRODUCTION_SSH_KEY`, `PRODUCTION_DB_HOST`, `PRODUCTION_DB_PASSWORD`이다. 선택 시 `PRODUCTION_USER`, `PRODUCTION_DB_USER`, `PRODUCTION_DB_NAME`을 설정할 수 있다. SSH/SCP 대상은 운영 호스트만이며(`PROD_SSH_JUMP_HOST` 미사용), `deploy-procedures-prod.yml`(개발 서버 경유·DEV_DB)과 목적이 다르다.
