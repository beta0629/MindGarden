 ...
 *         FullCalendar 실제 동작에 맞춰 start 는 이전 달 일요일, activeStart 는 활성 월 1일.
 *  - F12: currentMonth 변경 → 새 API 호출 (activeStart 우선 SSOT).
 ...
 *  - F16: 2026-06-09 R3 (P0) 회귀 — April 보기 시 start=2026-03-29(일요일)일지라도
 *         activeStart=2026-04-01 로 month=4 호출 검증. activeStart 미전달 폴백 시
 *         start.getMonth() 기준 mid(15일) 산출.
 *
...
describe('IntegratedMatchingSchedule — 월별 상담사 COMPLETED 카운트 캐시·트리거', () => {
  ...
  test('F11: 동일 tenantId/year/month onMonthChange 재호출 → 추가 API 호출 없음 (캐시 hit)', async() => {
    ...
    // 동일 6월 으로 재트리거 (FullCalendar 실제 동작 시뮬레이션 — start 는 이전 달 일요일,
    // activeStart 는 6월 1일). setCurrentYear/Month 가 동일값이라 setter no-op,
  ...
  test('F12: currentMonth 변경 → 새 API 호출', async() => {
    ...
    // 7월로 이동 — activeStart=2026-07-01 이 SSOT 이므로 start 가 이전 달 일요일이어도 month=7.
    await act(async() => {
      mockUnifiedScheduleProps.onMonthChange({
  ...
  // ─── F16 ───────────────────────────────────────────────────────────
  test('F16 (R3 P0 회귀): April view → start=2026-03-29(이전 달 일요일)일지라도 activeStart=2026-04-01 로 month=4 호출', async() => {
    setupApiResponses({
      monthly: buildMonthlyResponse([{ consultantId: 11, consultantName: 'A', count: 5 }])
    });
    ...
    // FullCalendar dayGridMonth April 보기: start 는 표시 첫 셀(이전 달 일요일).
    // activeStart 는 활성 월 1일. R3 픽스 후에는 activeStart 가 SSOT 이므로 month=4 호출.
    await act(async() => {
      mockUnifiedScheduleProps.onMonthChange({
        start: new Date('2026-03-29T00:00:00.000Z'),
        end: new Date('2026-05-10T00:00:00.000Z'),
        activeStart: new Date('2026-04-01T00:00:00.000Z'),
        view: 'dayGridMonth'
      });
    });
  ...
  // ─── F16b 폴백 ─────────────────────────────────────────────────────
  test('F16b (R3 폴백): activeStart 미전달 시 start 의 15일 mid 기준으로 month 산출 (이전 동작 호환)', async() => {
    setupApiResponses({
      monthly: buildMonthlyResponse([{ consultantId: 11, consultantName: 'A', count: 5 }])
    });
