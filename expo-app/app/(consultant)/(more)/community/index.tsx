/**
 * 커뮤니티 피드 (상담사) — 탭, 게시글 카드, FAB, Pull-to-refresh
 *
 * @author MindGarden
 * @since 2026-05-12
 */
...
export default function ConsultantCommunityFeed() {
  ...
  const navigateToDetail = (postId: number) => {
    ...
    router.push(`/(consultant)/(more)/community/${postId}`);
  };
  ...
  const navigateToCreate = () => {
    ...
    router.push('/(consultant)/(more)/community/create');
  };
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.colors.bgMain }]} edges={['top']}>
      ...
      >
        <View style={{ flex: 1 }}>
          ...
          >
            칼럼·후기는 우선 이 기기(MMKV)에 저장됩니다. 서버 API(/api/v1/community) 연동 후
            동기화됩니다.
          </Text>
        </View>
        <Pressable
...
interface ConsultantPostCardProps {
  post: CommunityPost;
  index: number;
  isLiked: boolean;
  onPress: () => void;
...
function ConsultantPostCard({ post, index, isLiked, onPress, onLike }: ConsultantPostCardProps) {
  const theme = useTheme();
...
const postStyles = StyleSheet.create({
  ...
  authorText: { flex: 1, marginLeft: 8 },
  footerRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 16,
