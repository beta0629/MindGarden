...
- **주요 변경 사항**:
  ...
  - `ContentHeader` P0 패턴: 본문 `title` + `titleId` SSOT, `AdminCommonLayout` `title` prop 생략 (Wellness 포함)
...
### 2.2 WellnessManagement (G-14 P0 본문 제목 SSOT)
**[Before]**
```text
...
**[After — P0/P1 SSOT]**
```text
<AdminCommonLayout loading={false}>
  <ContentArea>
    <ContentHeader
      title={t('admin:wellnessMgmt.title')}
      titleId="wellness-management-page-title"
      subtitle="..."
      actions={...}
    />
    <main aria-labelledby="wellness-management-page-title">...</main>
...
* `AdminCommonLayout` `title` prop 생략(LNB 이중 제목 방지). `title={null}` 금지.
```
...
- **AdminCommonLayout**: `title` prop **생략** (LNB 이중 제목 방지). `loading`만 전달.
- **ContentHeader**: `title={t('admin:wellnessMgmt.title')}`, `titleId="wellness-management-page-title"`, `subtitle`, `actions`. `title={null}` **금지**.
- **loading**: 데이터 로드 전역 로딩 상태 (초기 `true` -> 데이터 응답 후 `false`)
- **children**: 기존 B0KlA 본문 구조 유지. `<main aria-labelledby={titleId}>`.
