...
**변경 요지 (v1.4.0)**: PII_KEY/IV 회전 자동화 인프라(PR #344 Phase 1) + 배치 워크플로(`rotate-pii-key.yml` Phase 2) 신설에 따라 §3.4 를 "자동화 범위 외"에서 "Phase 1 ~ Phase 4 자동화" 절차로 전면 개정. Dual-Read 무중단 회전 + chunk 배치 + DBA 사후 검증 + 구 키 180일 보존 정책 반영. 운영 runbook(`docs/operations/pii-rotation-runbook.md`) 및 dev dry-run 가이드 분리.
...
5. **DBA 24h 사전 통지** + 운영 시간대(09:00~22:00 KST) 회피 — 23:00~06:00 권장
