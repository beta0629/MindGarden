/**
 * 내담자 설정 화면 스모크: `/client/settings` 진입·루트 testid 가시성
 *
 * 자격 증명: `TEST_CLIENT_USERNAME` · `TEST_CLIENT_PASSWORD` 가 **모두** 설정된 경우에만 로그인 후 검증한다.
 *
 * testid (`ClientSettings.js`): `client-settings-page`
 */
...
test.describe('내담자 설정 화면 스모크', () => {
  ...
  test('/client/settings 에서 페이지 testid가 보인다', async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    const pageRoot = page.getByTestId('client-settings-page');
