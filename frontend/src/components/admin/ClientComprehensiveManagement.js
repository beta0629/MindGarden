...
const ClientComprehensiveManagement = ({ embedded = false, initialOpenUserId = null }) => {
    ...
    const { viewMode, setViewMode } = useViewModePreference({
        storageKey: buildViewModeStorageKey(resolveViewModeStorageScope(), CLIENT_VIEW_MODE_PAGE_ID),
    ...
    const loadCommonCodes = useCallback(async() => {
        ...
        try {
            ...
            setUserStatusOptions([
                ...
                // ⚠️ 표준화 2025-12-05: 하드코딩된 상태값을 공통코드에서 동적 조회하세요. getCommonCodes('STATUS_GROUP') 사용
                { codeValue: 'PENDING', codeLabel: t('admin:labels.pending') }
            ]);
            showError(t('admin:client.error.commonCodeLoad'));
        } finally {
    ...
    const loadMappings = useCallback(async() => {
        try {
            const response = await apiGet(API_ENDPOINTS.ADMIN.MAPPINGS.LIST, ADMIN_MAPPINGS_PAGED_LIST_QUERY);
    ...
    useEffect(() => {
        ...
        if (result.action === 'open') {
            ...
            handleClientPeek(result.entity);
    ...
    const clientFilterOptions = useMemo(() => {
        const opts = [{ value: 'all', label: t('admin:labels.all') }];
    ...
    const handlePasswordResetConfirm = useCallback(async(newPassword) => {
        ...
        try {
            ...
            if (response && (response.success !== false)) {
                ...
                throw new Error(response?.message || t('admin:client.error.passwordReset'));
            }
        } catch (error) {
            ...
            <AdminCommonLayout title={t('admin:client.pageTitle')}>
                <div className="mg-v2-ad-b0kla mg-v2-client-management">
                    <div className="mg-v2-ad-b0kla__container">
                        <ContentArea ariaLabel={t('admin:client.ariaContent')}>
                            <ContentHeader
        ...
        <>
            <ContentSection noCard>
                <div className="mg-v2-ad-b0kla__pill-toggle">
                            ...
                            >
                                {t('admin:client.tab.consultation')}
                            </MGButton>
                            <MGButton
                                type="button"
                                variant="primary"
                            ...
                            <div className="mg-v2-mapping-search-section__row">
                                <div className="mg-v2-mapping-search-section__input-wrap">
                                    <SearchInput
                                        ...
                                        placeholder={t('admin:client.search.placeholder')}
                                    />
                                </div>
