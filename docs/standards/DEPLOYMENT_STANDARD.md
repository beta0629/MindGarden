...
- **Runner / Cursor cloud**: 현재 `runs-on`은 `ubuntu-latest`. Cursor 클라우드 SSH·DB·self-hosted runner 배치·전환 체크리스트는 [`CURSOR_CLOUD_SELF_HOSTED_RUNNER_SSOT.md`](../deployment/CURSOR_CLOUD_SELF_HOSTED_RUNNER_SSOT.md)를 SSOT로 본다(러너 미등록 시 `runs-on`만 self-hosted로 바꾸지 않음).
...
**운영 MySQL에 표준 프로시저만 적용**할 때는 `deploy-procedures-production-mysql.yml`을 수동 실행한다(`workflow_dispatch`). 입력 `confirm`에 정확히 `CONFIRM`을 넣어야 하며, 필수 시크릿은 `PRODUCTION_HOST`, `PRODUCTION_SSH_KEY`, `PRODUCTION_DB_PASSWORD`이다. `PRODUCTION_DB_HOST`는 비우면 `PRODUCTION_HOST`와 동일 호스트로 접속한다(앱과 DB가 같은 머신이면 `PRODUCTION_DB_HOST=127.0.0.1`로 두면 `user@localhost` GRANT와 맞기 쉽다). `PRODUCTION_DB_USER` 기본값은 앱과 동일하게 `mindgarden`이다(미설정 시). MySQL 8에서 기존 프로시저가 `root` 등 SYSTEM_USER 로 정의된 경우 일반 계정으로 `DROP` 시 1227이 나므로, 재시도 순서는 **`PRODUCTION_DB_PROCEDURE_USER` / `PRODUCTION_DB_PROCEDURE_PASSWORD`(비우면 `PRODUCTION_DB_PASSWORD`)** → **`PRODUCTION_MYSQL_ROOT_PASSWORD`**(root) → **로컬에서 `root` 비밀번호가 앱 DB와 동일**한 경우 `127.0.0.1`·`DB_HOST` 순 `root` 재시도다(평소 배포는 `PRODUCTION_DB_USER`만 사용). 선택 시 `PRODUCTION_USER`, `PRODUCTION_DB_NAME`을 설정할 수 있다. SSH/SCP 대상은 운영 호스트만이며(`PROD_SSH_JUMP_HOST` 미사용), `deploy-procedures-prod.yml`(개발 서버 경유·DEV_DB)과 목적이 다르다.
...
## Runner · Cursor cloud (요약)

- 수동 허브 [`deploy.yml`](../../.github/workflows/deploy.yml) 포함 배포·검증 잡은 **현재 GitHub-hosted(`ubuntu-latest`)**.
- Cursor 클라우드 ↔ 개발/운영 SSH·운영 DB 연동 가능 → self-hosted runner를 Cursor VM/SSH 호스트에 둘 수 있음. 전환 시 labels·문서만 맞추고, 미등록 상태에서 `runs-on`만 바꾸지 않는다.
- SSOT: [`docs/deployment/CURSOR_CLOUD_SELF_HOSTED_RUNNER_SSOT.md`](../deployment/CURSOR_CLOUD_SELF_HOSTED_RUNNER_SSOT.md)
