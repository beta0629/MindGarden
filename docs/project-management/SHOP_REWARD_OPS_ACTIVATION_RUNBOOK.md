...
| 9 | `V20260522_001` | `V20260522_001__shop_reward_activate_tenant_components_backfill.sql` | ACTIVE 상담 계열 테넌트 `CLIENT_SHOP`·`CLIENT_REWARD`·`ADMIN_SHOP_CATALOG` 일괄 백필 |
| 10 | `V20260522_002` | `V20260522_002__shop_reward_default_components_onboarding.sql` | 신규 온보딩 기본 컴포넌트 (`ActivateDefaultComponents`) |
| 11 | `V20260523_001` | `V20260523_001__shop_catalog_sku_thumbnail_url.sql` | `shop_catalog_skus.thumbnail_url` (PLP/PDP 대표 이미지, nullable·저장 시 서비스 필수) |
...
-- (H) V20260523_001 — thumbnail_url (MVP+ PLP/PDP)
...
- **`@tenant_id`** — mysql 실행 전 `SET @tenant_id='…';` 로 바인딩 (OPS 스크립트·SOURCE SSOT). **`:subdomain`** 은 §2.2 조회 SQL에서 수동 치환.
- MindGarden **대표 tenant**에 MVP 3종을 켠다: `CLIENT_SHOP`, `CLIENT_REWARD`, `ADMIN_SHOP_CATALOG`.

**저장소 스크립트**: `scripts/ops/activate-shop-reward-tenant-components.sql` (`@tenant_id` 세션 변수·멱등 `NOT EXISTS`).
...
# 동일 세션: mysql 클라이언트에서 SET @tenant_id 후 SOURCE
```
...
```sql
-- SET @tenant_id ← §2.2 조회 결과 후 SOURCE scripts/ops/activate-shop-reward-tenant-components.sql
-- (본문은 저장소 스크립트 SSOT — @tenant_id 사용)
```
...
-- 기대: 3행, status = ACTIVE (activate 직후 동일 세션에서 SET @tenant_id 유지 시)
```
...
**파일**: `scripts/ops/seed-shop-demo-catalog.sql` (`@tenant_id` 세션 변수·CONSULTATION 1건·멱등 `NOT EXISTS`)

| 항목 | 내용 |
|------|------|
| `tenant_id` | **`@tenant_id`** — §2.2 조회 UUID를 `SET @tenant_id='…';` 후 SOURCE (저장소·Flyway 하드코딩 금지) |
| SKU | **CONSULTATION 1건** — `sku_code='DEV-CONSULT-DEMO-01'`, `catalog_category='CONSULTATION'`, `catalog_visible=1`, `active=1`, `is_deleted=0` |
| 썸네일·placeholder | `thumbnail_url` — dev QA용 상대 URL `/api/v1/files/shop-catalog-thumbnails/placeholder-dev-consult-demo.png` (정적 placeholder; 실 업로드는 어드민 `POST .../catalog-skus/{id}/thumbnail` 또는 전용 폼) |
| 멱등 | 동일 `tenant_id` + `sku_code` 존재 시 `INSERT` 스킵 (`NOT EXISTS`, `uk_shop_sku_tenant_code` 정합); 기존 행은 `thumbnail_url` 비어 있으면 동일 placeholder로 `UPDATE` |
...
```sql
SELECT sku_code, catalog_category, catalog_visible, active, unit_price_minor, thumbnail_url
FROM shop_catalog_skus
WHERE tenant_id = @tenant_id AND is_deleted = 0 AND catalog_visible = 1
LIMIT 5;
-- 기대: DEV-CONSULT-DEMO-01 thumbnail_url = '/api/v1/files/shop-catalog-thumbnails/placeholder-dev-consult-demo.png'
```

**thumbnail·placeholder URL (MVP+ SSOT)**

| 항목 | 값 |
|------|-----|
| DB 컬럼 | `shop_catalog_skus.thumbnail_url` (`V20260523_001`) |
| API 필드 | `thumbnailUrl` (admin·client catalog DTO 동일) |
| dev 시드 placeholder | `/api/v1/files/shop-catalog-thumbnails/placeholder-dev-consult-demo.png` |
| 업로드 저장 경로 | `uploads/shop-catalog-thumbnails/` → URL prefix `/api/v1/files/shop-catalog-thumbnails/` ([`ShopCatalogSkuConstants`](../../src/main/java/com/coresolution/consultation/constant/ShopCatalogSkuConstants.java)) |
| 기존 tenant backfill | 어드민에서 실이미지 업로드 또는 OPS로 `thumbnail_url` 갱신; 빈 값은 신규 저장 시 API **400** |

**선행**: Flyway P2 `002`~`007`, `V20260520_001`, **`V20260523_001`**, §2 TenantComponent 활성화. **후속**: §4 Client C1~C2, §5 Playwright.
...
| 2 | **OPS activate** — `SET @tenant_id` + `SOURCE scripts/ops/activate-shop-reward-tenant-components.sql` (§2.2 조회 UUID) | ☐ |
...
**전제**: `develop` backend+frontend dev deploy **success** · `https://dev.core-solution.co.kr/actuator/health` **200**. `tenant_id`·`subdomain`는 저장소·Flyway에 박지 않는다 — 아래 **0)** 조회 UUID를 `SET @tenant_id='…'` 로만 바인딩한다. `api-*.json` 커밋 금지.
...
| 2 | **Flyway P2** — §1 `002`~`007` + `V20260520_001` + **`V20260523_001`** 적용·검증 SQL 통과 |
