...
Flyway `V20260519_003`은 **`component_catalog` 시드만** 수행한다. **테넌트별 on/off는 Flyway 밖 OPS SQL**이다 ([SHOP_REWARD §7](./SHOP_REWARD_PLATFORM_ORCHESTRATION.md)).
...
`src/main/resources/db/migration/V20260519_003__shop_reward_component_catalog_seed.sql` 하단 주석과 동일. **1회·멱등** (`NOT EXISTS`).
