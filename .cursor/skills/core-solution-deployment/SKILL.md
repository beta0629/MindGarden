...
- **환경 분리**: 개발(`release/dev`)·운영(`release/prod` / workflow_dispatch) 트리거와 배포 대상 서버가 표준과 일치하는지 확인. 수동 hub: `.github/workflows/deploy.yml`.
...
- [ ] 배포 브랜치(`release/dev` / `release/prod`) 및 수동 실행(workflow_dispatch / `deploy.yml` hub) 여부 확인
