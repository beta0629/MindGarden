...
public class TenantAccessControlService {
    ...
    public void validateTenantAccess(String requestedTenantId) {
        // 운영 포털(OPS) 관리자는 모든 테넌트에 접근 가능 (ADMIN ≠ OPS)
        if (hasOpsRole()) {
    ...
    public void validateOpsAccess() {
        if (!hasOpsRole()) {
            ...
            throw new AccessDeniedException("운영 포털 접근 권한이 없습니다 (OPS 역할 필요)");
        }
    ...
    /**
     * 현재 사용자가 운영 포털(OPS) 역할을 가지고 있는지 확인합니다.
     * ROLE_ADMIN 은 OPS 로 취급하지 않는다 (크로스 테넌트 PG CRUD 차단).
     *
     * @return OPS 역할 보유 여부
     */
    public boolean hasOpsRole() {
        ...
        return authorities.stream()
                .anyMatch(auth -> SecurityRoleConstants.ROLE_OPS.equals(auth.getAuthority()));
    }
