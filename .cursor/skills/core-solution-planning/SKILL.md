...
description: 전반 기획 절차·산출물·서브에이전트 가동 계획. 프로젝트·펜슬(B0KlA)·아토믹 디자인·LNB/GNB·색상 등 기획 필수 지식, Expo 앱 포함 시 캡슐화·모듈화 및 프론트·백엔드 기본 전제(§0.0.1), 화면설계서 작성 및 디자이너 전달 방법 포함.
---
...
## 0.-1. 기획·위임 전 — 스킬 선독 (필수)

- **분배실행·위임 Task를 띄우기 전에** 본 스킬과 작업 유형 스킬을 **먼저 읽는다.** UI·프론트면 `/core-solution-frontend` · `/core-solution-standardization` · (스타일이면) `/core-solution-design-system-css`.
- Task 프롬프트 첫머리에 **「관련 스킬 선독 후 작업」** 과 스킬 경로·**디자인 표준 준수**를 명시한다.
- **프론트 디자인은 프로젝트 디자인 표준만** — 임의 시안·색을 기획에 넣지 않는다.
...
### 0.0.1 모바일(Expo) 앱 — 캡슐화·모듈화 및 프론트·백엔드 기본

**`expo-app/`(Expo·React Native)** 도 웹 `frontend/`와 **코드베이스는 분리**되어 있으나, 기획·Phase·분배실행 시 **Core Solution과 동일한 기본 전제**를 명시한다. “웹만”으로 기획하지 말고, 앱이 범위에 들어가면 아래를 기획서·위임문에 적는다.

| 전제 | 기획 시 명시할 내용 |
|------|---------------------|
| **캡슐화·모듈화** | `/core-solution-encapsulation-modularization`과 **동일**하게 적용. 라우트·화면(`app/`) vs **Atoms/Molecules/Organisms**(`src/components/`) vs **API 계층**(`src/api/`, 훅·`client`) vs **도메인 서비스**(`src/services/`) vs **상태**(`src/stores/`) 등 **경계·책임**을 Phase·산출물에 쓴다. |
| **프론트엔드 기본** | 앱도 **아토믹·토큰·공통 패턴**이 기본이다. `/core-solution-atomic-design`, Expo용 테마·토큰 경로(`expo-app/src/theme` 등), API 연동·에러·테넌트 헤더 규칙은 **`/core-solution-api`**, 표시·UI 표준은 **`/core-solution-frontend`**(웹·앱 공통 원칙·모바일 적용분)를 코더·디자이너 전달문에 넣는다. |
| **백엔드 기본** | **앱 전용 백엔드 예외 없이** Java/Spring이 권위이다. **`/core-solution-backend`**, **멀티테넌트**는 **`/core-solution-multi-tenant`**, DB·API 선행은 **`/core-solution-database-first`**를 전제로 “동일 API·동일 `tenantId` 경계”로 기획한다. |

- **기획서·분배실행 표**에 “Expo 포함 시: 화면 경로(`expo-app/app/…`), 공유 컴포넌트·훅 경로, 백엔드 엔드포인트/Phase”를 웹과 **대칭**되게 적어 추측 구현을 막는다.
- **참조**: `docs/project-management/EXPO_NATIVE_APP_PLAN.md`(있을 경우), `docs/design-system/v2/`의 클라이언트·앱 스펙.
...
- **운영 반영 플랜**(Go-Live·프로덕션 배포 준비) 수립·오케스트레이션 — 체크리스트·데이터 선별·서브에이전트(shell·core-debugger·core-tester·문서정리 등) 분배실행
...
### 운영 반영 플랜 오케스트레이션 (core-planner 주관)

운영 반영·Go-Live·프로덕션 배포 준비를 요청받으면 **core-planner가 주관**하여 오케스트레이션한다.

| 참조 문서 | 용도 |
|-----------|------|
| `docs/운영반영/PRE_PRODUCTION_GO_LIVE_CHECKLIST.md` | Go-Live 체크리스트·문서 계층·§1~§7 항목 |
| `docs/deployment/PRODUCTION_ESSENTIAL_DATA.md` | DB·데이터 선별(필수 vs 비필수, Flyway vs 덤프 금지, 온보딩으로 테넌트 생성) |
| `/core-solution-deployment` 스킬 | 배포 표준·스크립트·환경 |

**분배실행 예**: explore(현황 조사) → shell(스크립트·배포·로그) → core-debugger(리스크·롤백 확인) → core-tester(스모크·검증) → generalPurpose+문서정리(회의 결론·체크리스트 갱신). 결과를 기획이 취합해 사용자에게 최종 보고.
...
- **Expo·모바일 포함 시**: §0.0.1에 따라 **`/core-solution-encapsulation-modularization`** 및 API·테넌트·백엔드 기본 스킬을 태스크에 명시하고, 웹과 **동일 API·동일 경계** 전제를 적는다.
...
- [ ] **Expo/모바일이 범위에 포함되면** §0.0.1대로 캡슐화 경계·프론트·백엔드 기본 스킬 참조·웹과 동일 API·테넌트 전제가 기획서·위임문에 적혀 있는가?
