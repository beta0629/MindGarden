/**
 * ResetPassword — password field mobile layout 회귀 격리 테스트.
 *
...
describe('ResetPassword 정적 계약 — password field mobile layout', () => {
  const fs = require('fs');
  const path = require('path');

  const jsSource = fs.readFileSync(
    path.resolve(__dirname, '..', 'ResetPassword.js'),
    'utf8'
  );
  ...
  test('AuthPageCommon.css contains padding-right rule for password input.', () => {
    expect(cssSource).toMatch(
      /\.mg-v2-auth-container\s+\.mg-v2-password-wrapper\s+\.mg-v2-input[\s\S]*?padding-right:\s*48px/
    );
  });

  test('AuthPageCommon.css auth inputs use auto height and touch-friendly min-height.', () => {
    expect(cssSource).toMatch(
      /\.mg-v2-auth-container\s+\.mg-v2-input\s*,[\s\S]*?height:\s*auto;[\s\S]*?min-height:\s*44px;/
    );
  });
});
