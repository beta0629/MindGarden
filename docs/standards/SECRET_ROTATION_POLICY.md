...
**변경 요지 (v1.4.1)**: PII 회전 컨트롤러 권한 가드를 옵션 3+1 하이브리드 (OPS Authority + HQ 테넌트 가드) 로 강화 — `ops-portal-migration` Phase 1. `@PreAuthorize("hasRole('OPS')")` + `OpsTenantConstants#isHqTenant(...)` 메서드별 자체 검증 병행 (Defense in Depth). `MINDGARDEN_HQ_TENANT_ID` 환경변수 fail-fast 사전 조건 추가. 자세한 배경은 `docs/project-management/OPS_PORTAL_MIGRATION_PLAN.md` §6.
**이전 변경 (v1.4.0)**: PII_KEY/IV 회전 자동화 인프라(PR #344 Phase 1) + 배치 워크플로(`rotate-pii-key.yml` Phase 2) 신설에 따라 §3.4 를 "자동화 범위 외"에서 "Phase 1 ~ Phase 4 자동화" 절차로 전면 개정. Dual-Read 무중단 회전 + chunk 배치 + DBA 사후 검증 + 구 키 180일 보존 정책 반영. 운영 runbook(`docs/operations/pii-rotation-runbook.md`) 및 dev dry-run 가이드 분리.
**이전 변경 (v1.3.0)**: 회전 자동화 v2 — `ROTATION_SECRETS_PAT` 만료 알림(`check-pat-expiry.yml`) + 분기 자동 회전 트리거(`quarterly-secret-rotation-trigger.yml`) + prod Environment protection 가이드(`docs/operations/prod-environment-protection.md`) 신설. §8 "자동화" 신설로 SSOT 분리, 후속 §9~§12 재번호.
...
**자동화 범위 (v1.4.0)**: `JWT_SECRET` (§3.1), `KAKAO/NAVER` (§3.2), `DB_PASSWORD` (§3.3), `PII_KEY/IV` (§3.4 Phase 1~4) 모두 워크플로 자동화. 회전 인프라(만료 알림·분기 자동 트리거·prod Environment 보호)는 §8 자동화 섹션 단일 SSOT 로 관리한다. OAuth (Google·Apple) 는 외부 콘솔 동기화 의존으로 수동 유지.
...
### 3.4 PII KEY / IV — Phase 1 ~ Phase 4 자동화 (v1.4.0)

> **회전 시 기존 데이터 재암호화 마이그레이션이 필요**하므로 다른 secret 과 달리 4단계로 분리한다. **Dual-Read 인프라**(`PersonalDataEncryptionUtil` 다중 키)로 무중단 회전을 보장한다.
...
| `PiiKeyRotationAdminController` | **OPS Authority + HQ 테넌트 가드 (옵션 3+1 하이브리드)** 전용 admin endpoint 4종 (start/progress/resume/cancel) — `@PreAuthorize("hasRole('OPS')")` + `OpsTenantConstants#isHqTenant(...)` 자체 검증 병행 (`docs/standards/ROLE_STANDARD.md` §3.2 + `docs/project-management/OPS_PORTAL_MIGRATION_PLAN.md` §6) | `src/main/java/com/coresolution/core/controller/PiiKeyRotationAdminController.java` (PR #344, Phase 1 OPS guard) |
| `OpsTenantConstants` | 본사(HQ) 테넌트 ID SSOT — `MINDGARDEN_HQ_TENANT_ID` 환경변수 주입 + `@PostConstruct` fail-fast (Phase 1 — `ops-portal-migration`) | `src/main/java/com/coresolution/core/constant/OpsTenantConstants.java` (Phase 1) |
...
| dev dry-run 가이드 | dev readonly 검증 절차 | `docs/operations/pii-rotation-dry-run-guide.md` (본 PR) |

#### Phase 별 책임 / 자동화 범위

| Phase | 책임 | 상태 변경 | 자동화 | 워크플로 input.phase |
|---|---|---|---|---|
| **Phase 1 verify** | 운영팀 | 0 (state mutation 없음) | 워크플로 자동 (BE health + admin endpoint 가용성) | `phase1-verify` |
| **Phase 2 batch** | 운영팀 | DB row UPDATE (재암호화) | 워크플로 자동 (start + polling 60s × 최대 6h) | `phase2-batch` |
| **Phase 3 verify** | DBA + 운영팀 | 0 (잔여 SELECT 만) | 워크플로가 SQL 명세 출력 + DBA 수동 실행 | `phase3-verify` |
| **Phase 4 finalize** | 운영팀 | GH Secrets CSV 수정 (구 키 제거, 180일 후) | 이력 PR 자동, CSV 수정 수동 | `phase4-finalize` |
| **batch-status** | 운영팀 | 0 (관찰만) | 워크플로 자동 (/progress 1회 조회) | `batch-status` |
...
3. **admin JWT 발급**: **본사(HQ) 테넌트 + OPS Authority** 보유 계정으로 JWT 발급 → `secrets.PII_ROTATION_ADMIN_TOKEN` 등록 (runbook §5, 1주일 이하 만료). 외부 테넌트 ADMIN 토큰은 컨트롤러의 `OpsTenantConstants` HQ 가드가 403 으로 차단한다 (Defense in Depth — OPS_PORTAL_MIGRATION_PLAN §6).
4. **HQ 테넌트 ID 환경변수 사전 등록**: `MINDGARDEN_HQ_TENANT_ID` GH Secret (env=`dev` / env=`prod` 각각) 등록 필수. 미설정 시 `OpsTenantConstants#validate()` 가 부트 자체를 fail-fast 시켜 회전 endpoint 진입 자체가 차단된다.
5. **BE 설정 사전 확인**:
...
6. **DBA 24h 사전 통지** + 운영 시간대(09:00~22:00 KST) 회피 — 23:00~06:00 권장

#### 실행 절차 (요약 — 자세한 절차는 runbook §3 참조)

1. **신 키 등록** (수동) — GH Secrets `PERSONAL_DATA_ENCRYPTION_KEYS` CSV append + 활성 키 ID 갱신 + deploy 재실행
2. **Phase 1 verify** 워크플로 dispatch — BE health + admin endpoint 가용성 확인
3. **Phase 2 batch** 워크플로 dispatch — 1회 1테이블 권장 (`users` → `clients` → `accounts` → `branches` → `dormant`):
   - confirm 3단계 (`ROTATE` + `PROD_ROTATE` (prod 시) + `DBA_APPROVED`)
   - chunk_size 50~500 권장, default 100
   - 60초 간격 polling, 최대 6시간 (대규모 데이터 대비)
   - 실패 chunk 격리 + 다음 chunk 계속 진행 (FAILED 마킹)
4. **Phase 3 verify** 워크플로 dispatch — 잔여 SQL 출력 → DBA readonly 수동 실행 → `not_rotated = 0` 확인
5. **Phase 4 finalize** 워크플로 dispatch — `secret-rotation-history.md` 이력 PR 자동 생성
6. **24h 모니터링** GREEN 유지 — 에러율 / 알람 / 복호화 실패 row 0
7. **180일 보존 후 구 키 폐기** (수동) — CSV 에서 구 키 entry 제거 + 폐기 PR

#### 핵심 안전 원칙 (절대 금지)

- **PERSONAL_DATA_ENCRYPTION_KEYS CSV 자동 수정 금지** — 본 워크플로 permissions 에 secret:write 없음. CSV 갱신·구 키 폐기는 운영팀 수동만 (정책 §11 — secret 자동화 분리 원칙)
- **단일 키 운영 중 회전 금지** — 신 키 등록 직후 다중 키 (vNEW + vCURRENT) 병존 상태에서만 회전 시작 (Dual-Read 호환)
- **IV 단독 회전 금지** — KEY 와 IV 는 항상 동시 회전 (복호화 호환 깨짐)
- **180일 이전 강제 구 키 폐기 금지** — 휴면 PII (180일 inactivity 후 복호화) 호환 보장
- **평문 PII 응답·로그 노출 0** — `PiiRotationProgressResponse` / `PiiRotationResult` DTO 가드 + 워크플로 정규식 가드
- **admin JWT 평문 출력 금지** — `::add-mask::` 즉시 마스킹, 만료 1주일 이하
- **확정 회전 직전 dev readonly 1회 dry-run 검증 필수** — runbook §3 + dry-run 가이드 절차
...
- [`.github/workflows/rotate-pii-key.yml`](../../.github/workflows/rotate-pii-key.yml) — PII KEY 회전 배치 워크플로 (PII Phase 2, §3.4 v1.4.0)
- [`docs/operations/pii-rotation-runbook.md`](../operations/pii-rotation-runbook.md) — PII KEY 회전 운영 runbook (PII Phase 2, §3.4 v1.4.0)
- [`docs/operations/pii-rotation-dry-run-guide.md`](../operations/pii-rotation-dry-run-guide.md) — PII KEY 회전 dev dry-run 가이드 (PII Phase 2, §3.4 v1.4.0)
...
| 2026-06-15 | 1.4.0 | PII KEY 회전 Phase 2 — 배치 실행 워크플로(`rotate-pii-key.yml`, Phase 1~4 dispatch + polling 60s × 6h) 신설. §3.4 를 "자동화 범위 외"에서 "Phase 1 ~ Phase 4 자동화" 절차로 전면 개정. Dual-Read 무중단 회전 + chunk 배치 + DBA 사후 검증 + 구 키 180일 보존 정책 반영. 운영 runbook(`docs/operations/pii-rotation-runbook.md`) 및 dev dry-run 가이드(`docs/operations/pii-rotation-dry-run-guide.md`) 분리. | MindGarden |
| 2026-06-15 | 1.4.1 | `ops-portal-migration` Phase 1 — PII 회전 컨트롤러 권한 가드를 **OPS Authority + HQ 테넌트 가드 (옵션 3+1 하이브리드)** 로 강화. `PiiKeyRotationAdminController` 클래스 레벨 `@PreAuthorize` 를 `hasRole('ADMIN')` → `hasRole('OPS')` 전환 + 각 메서드 진입부에 `OpsTenantConstants#isHqTenant(...)` 자체 검증 추가 (Defense in Depth). `OpsTenantConstants` 신설 (`MINDGARDEN_HQ_TENANT_ID` 환경변수 + `@PostConstruct` fail-fast). 운영 반영 전 `MINDGARDEN_HQ_TENANT_ID` GH Secret 등록 (env=dev/prod) 필수. | MindGarden |
